...
#error \
    "This platform does not support subinterpreters, do not include this file."
#endif
...
/// Activate the subinterpreter and acquire its GIL, while also releasing any
/// GIL and interpreter currently held. Upon exiting the scope, the previous
/// subinterpreter (if any) and its associated GIL are restored to their state
/// as they were before the scope was entered.
class subinterpreter_scoped_activate {
 public:
  ...
  subinterpreter_scoped_activate(subinterpreter_scoped_activate const &) =
      delete;
  subinterpreter_scoped_activate &operator=(subinterpreter_scoped_activate &) =
      delete;
  subinterpreter_scoped_activate &operator=(
      subinterpreter_scoped_activate const &) = delete;
...
class subinterpreter {
 public:
  /// empty/unusable, but move-assignable.  use create() to create a
  /// subinterpreter.
  ...
  /// @note This function acquires (and then releases) the main interpreter GIL,
  /// but the main interpreter and its GIL are not required to be held prior to
  ...
  static subinterpreter create(PyInterpreterConfig const &cfg) {
    ...
    subinterpreter result;
    {
      ...
      {
        /*
        Several internal CPython modules are lacking proper subinterpreter
        support in 3.12 even though it is "stable" in that version.  This most
        commonly seems to cause crashes when two interpreters concurrently
        initialize, which imports several things (like builtins, unicode,
        codecs).
        */
      ...
      // this doesn't raise a normal Python exception, it provides an exit()
      // status code.
      ...
      detail::get_internals();  // initialize internals.tstate, amongst other
                                // things...

      // In 3.13+ this state should be deleted right away, and the memory will
      // be reused for the next threadstate on this interpreter. However,
      // on 3.12 we cannot do that, we must keep it around (but not use it) ...
      // see destructor.
  ...
  /// Calls create() with a default configuration of an isolated interpreter
  /// that disallows fork, exec, and Python threads.
  ...
  ~subinterpreter() {
    ...
    // Python 3.12 requires us to keep the original PyThreadState alive until we
    // are ready to destroy the interpreter.  We prefer to use that to destroy
    // the interpreter.
#if PY_VERSION_HEX < 0x030D0000
    // The tstate passed to Py_EndInterpreter MUST have been created on the
    // current OS thread.
    bool same_thread = false;
#ifdef PY_HAVE_THREAD_NATIVE_ID
    same_thread =
        PyThread_get_thread_native_id() == creation_tstate_->native_thread_id;
#endif
    if (same_thread) {
      ...
      // We can use the one we just created, so we must delete the creation
      // state.
    ...
    // Internals always exists in the subinterpreter, this class enforces it
    // when it creates the subinterpreter. Even if it didn't, this only creates
    // the pointer-to-pointer, not the internals themselves.
    ...
    // It's possible for the  internals to be created during endinterpreter
    // (e.g. if a py::capsule calls `get_internals()` during destruction), so we
    // destroy afterward.
  ...
  /// Get a handle to the main interpreter that can be used with
  /// subinterpreter_scoped_activate Note that destructing the handle is a noop,
  /// the main interpreter can only be ended by py::finalize_interpreter()
  ...
  int64_t id() const {
    ...
    return -1;  // CPython uses one-up numbers from 0, so negative should be
                // safe to return here.
  }

  /// Get the interpreter's state dict.  This interpreter's GIL must be held
  /// before calling!
  dict state_dict() {
    return reinterpret_borrow<dict>(PyInterpreterState_GetDict(istate_));
  }

  /// abandon cleanup of this subinterpreter (leak it). this might be needed
  /// during finalization...
...
inline subinterpreter_scoped_activate::~subinterpreter_scoped_activate() {
  if (simple_gil_) {
    // We were on this interpreter already, so just make sure the GIL goes back
    // as it was
    ...
    if (tstate_) {
      ...
      if (detail::get_thread_state_unchecked() != tstate_) {
        pybind11_fail(
            "~subinterpreter_scoped_activate: thread state must be current!");
      }
#endif
