...
PYBIND11_NOINLINE module_
import_numpy_core_submodule(const char *submodule_name) {
  ...
  std::string numpy_core_path =
      major_version >= 2 ? "numpy._core" : "numpy.core";
...
struct npy_api {
  ...
  enum constants {
    ...
    // NPY_BITSOF_LONG, NPY_BITSOF_LONGLONG, NPY_BITSOF_INT, NPY_BITSOF_SHORT,
    // NPY_BITSOF_CHAR and assigns the alias to the first matching size, so we
    ...
    NPY_FLOAT64_ = platform_lookup<double, double, float, long double>(
        NPY_DOUBLE_, NPY_FLOAT_, NPY_LONGDOUBLE_),
    NPY_COMPLEX64_ =
        platform_lookup<std::complex<float>, std::complex<double>,
                        std::complex<float>, std::complex<long double>>(
            NPY_DOUBLE_, NPY_FLOAT_, NPY_LONGDOUBLE_),
    NPY_COMPLEX128_ =
        platform_lookup<std::complex<double>, std::complex<double>,
                        std::complex<float>, std::complex<long double>>(
            NPY_DOUBLE_, NPY_FLOAT_, NPY_LONGDOUBLE_),
...
// This table normalizes typenums by mapping NPY_INT_, NPY_LONG, ... to
// NPY_INT32_, NPY_INT64, ... This is needed to correctly handle situations
// where multiple typenums map to the same type, e.g. NPY_LONG_ may be
// equivalent to NPY_INT_ or NPY_LONGLONG_ despite having a different typenum.
// The normalized typenum should always match the values used in
// npy_format_descriptor. If you change this code, please review `enum
// constants` above.
...
using is_pod_struct =
    ...
    (__GLIBCXX__ < 20150422 || __GLIBCXX__ == 20150426 ||  \
     __GLIBCXX__ == 20150623 || __GLIBCXX__ == 20150626 || \
...
struct type_caster<numpy_scalar<T>> {
  ...
  static handle &target_type() {
    static handle tp =
        npy_api::get().PyArray_TypeObjectFromType_(type_info::typenum);
    return tp;
  }

  static handle &target_dtype() {
    static handle tp =
        npy_api::get().PyArray_DescrFromType_(type_info::typenum);
    return tp;
  }
  ...
  static handle cast(numpy_scalar<T> src, return_value_policy, handle) {
    return npy_api::get().PyArray_Scalar_(&src.value, target_dtype().ptr(),
                                          nullptr);
  }
};
...
class dtype : public object {
 public:
  ...
  /// Type number of dtype. Note that different values may be returned for
  /// equivalent types, e.g. even though ``long`` may be equivalent to ``int``
  /// or ``long long``, they still have different type numbers. Consider using
  /// `normalized_num` to avoid this.
  int num() const {
  ...
  /// Type number of dtype, normalized to match the return value of `num_of` for
  /// equivalent types. This function can be used to write switch statements
  /// that correctly handle equivalent types with different type numbers.
 ...
 public:
  ...
  array(const pybind11::dtype &dt, ShapeContainer shape,
    ...
    auto tmp = reinterpret_steal<object>(api.PyArray_NewFromDescr_(
        ...
        // Use reinterpret_cast for PyPy on Windows (remove if fixed, checked
        // on 7.3.1)
  ...
  array view(const std::string &dtype) {
    ...
    auto new_view = reinterpret_steal<array>(api.PyArray_View_(
        m_ptr, dtype::from_args(pybind11::str(dtype)).release().ptr(),
        nullptr));
    if (!new_view) {
      throw error_already_set();
    }
...
class array_t : public array {
 ...
 public:
  ...
  explicit array_t(ssize_t count, const T *ptr = nullptr,
                   handle base = handle())
      : array({count}, {}, ptr, base) {}
  ...
  static bool check_(handle h) {
    ...
    return api.PyArray_Check_(h.ptr()) &&
           api.PyArray_EquivTypes_(detail::array_proxy(h.ptr())->descr,
                                   dtype::of<T>().ptr()) &&
           detail::check_flags(h.ptr(),
                               ExtraFlags & (array::c_style | array::f_style));
  }
...
template <typename T>
struct npy_format_descriptor<
    T, enable_if_t<is_same_ignoring_cvref<T, PyObject *>::value ||
...
PYBIND11_NOINLINE void register_structured_dtype(
    any_container<field_descriptor> fields, const std::type_info &tinfo,
  ...
  numpy_internals.registered_dtypes[tindex] = {dtype_ptr,
                                               std::move(format_str)};
...
#ifdef __CLION_IDE__  // replace heavy macro with dummy code for the IDE
                      // (doesn't affect code)
...
#if defined(_MSC_VER) && !defined(__clang__)  // MSVC is not as eager to expand
                                              // macros, hence this workaround
...
struct vectorize_returned_array {
  ...
  static Type create(broadcast_trivial trivial,
 ...
 public:
  template <typename T,
            ...
            typename = detail::enable_if_t<!std::is_same<
                vectorize_helper, typename std::decay<T>::type>::value>>
 ...
 private:
  ...
  object run(typename vectorize_arg<Args>::type &...args,
    ...
    if (trivial == broadcast_trivial::non_trivial) {
      apply_broadcast(buffers, params, mutable_data, size, shape, i_seq, vi_seq,
                      bi_seq);
    } else {
...
template <typename Func, typename Return, typename... Args>
vectorize_helper<Func, Return, Args...> vectorize_extractor(
    const Func &f, Return (*)(Args...)) {
...
struct handle_type_name<array_t<T, Flags>> {
  static constexpr auto name =
      io_name("typing.Annotated[numpy.typing.ArrayLike, ",
...
detail::vectorize_helper<Return (*)(Args...), Return, Args...> vectorize(
    Return (*f)(Args...)) {
...
// Intentionally no &&/const&& overloads: vectorized method calls operate on the
// bound Python instance and should not consume/move-from self. Vectorize a
// class method (non-const, lvalue ref-qualified):
template <
    typename Return, typename Class, typename... Args,
    typename Helper = detail::vectorize_helper<
        decltype(std::mem_fn(std::declval<Return (Class::*)(Args...) &>())),
...
template <typename Return, typename Class, typename... Args,
          typename Helper = detail::vectorize_helper<
              decltype(std::mem_fn(
                  std::declval<Return (Class::*)(Args...) const &>())),
              Return, const Class *, Args...>>
Helper vectorize(Return (Class::*f)(Args...) const &) {
  return Helper(std::mem_fn(f));
}
...
template <typename Return, typename Class, typename... Args,
          typename Helper = detail::vectorize_helper<
              decltype(std::mem_fn(
                  std::declval<Return (Class::*)(Args...) noexcept>())),
              Return, Class *, Args...>>
...
template <typename Return, typename Class, typename... Args,
          typename Helper = detail::vectorize_helper<
              decltype(std::mem_fn(
                  std::declval<Return (Class::*)(Args...) const noexcept>())),
              Return, const Class *, Args...>>
