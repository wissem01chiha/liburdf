...
// In this case, we can store the result globally, because there is only a
// single interpreter.
//
// The life span of the stored result is the entire process lifetime. It is
// leaked on process termination to avoid destructor calls after the Python
...
class gil_safe_call_once_and_store {
 public:
  ...
  // called.
  //
  // NOTE: The second parameter (finalize callback) is intentionally unused when
  // subinterpreter support is disabled. In that case, storage is process-global
  // and intentionally leaked to avoid calling destructors after the Python
  // interpreter has been finalized.
  template <typename Callable>
  gil_safe_call_once_and_store &call_once_and_store_result(
      Callable &&fn, void (*)(T &) /*unused*/ = nullptr) {
    if (!is_initialized_) {  // This read is guarded by the GIL.
      // Multiple threads may enter here, because the GIL is released in the
      // next line and CPython API calls in the `fn()` call below may release
      ...
      std::call_once(once_flag_, [&] {
        ...
        ::new (storage_)
            T(fn());             // fn may release, but will reacquire, the GIL.
  ...
  gil_safe_call_once_and_store &operator=(
      const gil_safe_call_once_and_store &) = delete;
  gil_safe_call_once_and_store &operator=(gil_safe_call_once_and_store &&) =
      delete;

 private:
  // The global static storage (per-process) when subinterpreter support is
  // disabled.
...
// In this case, we should store the result per-interpreter instead of globally,
// because each subinterpreter has its own separate state. The cached result may
// not shareable across interpreters (e.g., imported modules and their members).
...
#define PYBIND11_CALL_ONCE_STORAGE_KEY_PREFIX \
  PYBIND11_INTERNALS_ID "_call_once_storage__"

// The life span of the stored result is the entire interpreter lifetime. An
// additional `finalize_fn` can be provided to clean up the stored result when
...
class gil_safe_call_once_and_store {
 public:
  ...
  template <typename Callable>
  gil_safe_call_once_and_store &call_once_and_store_result(
      Callable &&fn, void (*finalize_fn)(T &) = nullptr) {
    if (!is_last_storage_valid()) {
      ...
      // All threads will observe `is_initialized_by_at_least_one_interpreter_`
      // as true here.
    }
    // Intentionally not returning `T &` to ensure the calling code is
    // self-documenting.
 ...
 private:
  ...
  // Get the unique key for this storage instance in the interpreter's state
  // dict. The return type should not be `py::str` because PyObject is
  // interpreter-dependent.
  std::string get_storage_key() const {
    // The instance is expected to be global static, so using its address as
    // unique identifier. The typical usage is like:
    //
  ...
  // The storage is interpreter-dependent and will not be shared across
  // interpreters.
  ...
  // interpreters. This is only valid if there is a single interpreter.
  // Otherwise, it is not used. WARNING: We cannot use thread local cache
  // similar to `internals_pp_manager::internals_p_tls`
  //          because the thread local storage cannot be explicitly invalidated
  //          when interpreters are destroyed (unlike `internals_pp_manager`
  //          which has explicit hooks for that).
  T *last_storage_ptr_ = nullptr;
  // This flag is true if the value has been initialized by any interpreter (may
