...
// See PR #5448. This warning suppression is needed for the PYBIND11_OVERRIDE
// macro family. NOTE that this is NOT embedded in a push/pop pair because that
...
inline std::string replace_newlines_and_squash(const char *text) {
  ...
  // Replace characters in whitespaces array with spaces and squash consecutive
  // spaces
  while (*text != '\0') {
...
inline std::string generate_function_signature(
    const char *type_caster_name_field, detail::function_record *func_rec,
    const std::type_info *const *types, size_t &type_index, size_t &arg_index) {
  std::string signature;
  bool is_starred = false;
  // `is_return_value.top()` is true if we are currently inside the return type
  // of the signature. Using `@^`/`@$` we can force types to be arg/return types
  // while `@!` pops back to the previous state.
  std::stack<bool> is_return_value({false});
  // The following characters have special meaning in the signature parsing.
  // Literals containing these are escaped with `!`.
  ...
  for (const auto *pc = type_caster_name_field; *pc != '\0'; ++pc) {
    ...
    if (c == '{') {
      ...
      if (arg_index < func_rec->args.size() && func_rec->args[arg_index].name) {
        ...
        signature +=
            "arg" + std::to_string(arg_index - (func_rec->is_method ? 1 : 0));
      }
      signature += ": ";
    } else if (c == '}') {
      ...
      if (func_rec->nargs_pos_only > 0 &&
          (arg_index + 1) == func_rec->nargs_pos_only) {
        signature += ", /";
      }
      if (!is_starred) {
        arg_index++;
      ...
      if (auto *tinfo = detail::get_type_info(*t)) {
        ...
        signature += th.attr("__module__").cast<std::string>() + "." +
                     ...
                     detail::global_internals_native_enum_type_map_get_item(
                         *t)) {
        signature += th.attr("__module__").cast<std::string>() + "." +
                     th.attr("__qualname__").cast<std::string>();
        ...
        signature +=
            detail::quote_cpp_type_name(detail::clean_type_id(t->name()));
      }
    } else if (c == '!' && special_chars.find(*(pc + 1)) != std::string::npos) {
      // typing::Literal escapes special characters with !
      signature += *++pc;
    } else if (c == '@') {
      // `@^ ... @!` and `@$ ... @!` are used to force arg/return value type
      // (see typing::Callable/detail::arg_descr/detail::return_descr)
      if (*(pc + 1) == '^') {
      ...
      // For named arguments (py::arg()) with noconvert set, return value type
      // is used.
      ++pc;
      if (!is_return_value.top() && (!(arg_index < func_rec->args.size() &&
...
inline std::string generate_type_signature() {
  ...
  PYBIND11_DESCR_CONSTEXPR auto descr_types =
      decltype(caster_name_field)::types();
  // Create a default function_record to ensure the function signature has the
  // proper configuration e.g. no_convert.
  auto func_rec = function_record();
  size_t type_index = 0;
  size_t arg_index = 0;
  return generate_function_signature(caster_name_field.text, &func_rec,
                                     descr_types.data(), type_index, arg_index);
}
...
#define PYBIND11_READABLE_FUNCTION_SIGNATURE_EXPR                              \
  detail::const_name("(") + cast_in::arg_names + detail::const_name(") -> ") + \
      cast_out::name
...
class ReadableFunctionSignature {
 ...
 private:
  ...
  // unknown N.)
  static constexpr sig_type sig() {
    return PYBIND11_READABLE_FUNCTION_SIGNATURE_EXPR;
  }
...
class cpp_function : public function {
 public:
  ...
  /// Construct a cpp_function from a class method (non-const, rvalue
  /// ref-qualifier)
  template <typename Return, typename Class, typename... Arg, typename... Extra>
  // NOLINTNEXTLINE(google-explicit-constructor)
  cpp_function(Return (Class::*f)(Arg...) &&, const Extra &...extra) {
  ...
  template <typename Return, typename Class, typename... Arg, typename... Extra>
  // NOLINTNEXTLINE(google-explicit-constructor)
  cpp_function(Return (Class::*f)(Arg...) const noexcept,
               const Extra &...extra) {
    initialize(
        [f](const Class *c, Arg... args) -> Return {
          return (c->*f)(std::forward<Arg>(args)...);
 ...
 protected:
  struct InitializingFunctionRecordDeleter {
    // `destruct(function_record, false)`: `initialize_generic` copies strings
    // and takes care of cleaning up in case of exceptions. So pass `false` to
    // `free_strings`.
    void operator()(detail::function_record *rec) { destruct(rec, false); }
  };
  using unique_function_record =
      std::unique_ptr<detail::function_record,
                      InitializingFunctionRecordDeleter>;
  ...
  void initialize(Func &&f, Return (*)(Args...), const Extra &...extra) {
    ...
    rec->nargs_pos =
        cast_in::args_pos >= 0
            ...
            : sizeof...(Args) - cast_in::has_kwargs;  // Will get reduced more
                                                      // if we have a kw_only
...

    {
      ...
      constexpr bool has_is_method =
          any_of<std::is_same<is_method, Extra>...>::value;
      // The implicit `self` argument is not present and not counted in method
      // definitions.
      ...
      static_assert(
          ((/* Need `py::arg("arg_name")` annotation in function/method. */
            has_arg_annotations) ||
           (/* Allow methods with no arguments `def method(self, /): ...`.
            ...
            is_method_with_self_arg_only)) ||
              !has_pos_only_args,
          "py::pos_only requires the use of argument annotations (for "
          "docstrings "
...
#else
    PYBIND11_DESCR_CONSTEXPR auto types =
        std::decay<decltype(signature)>::type::types();
#endif
  ...
  void initialize_generic(unique_function_record &&unique_rec, const char *text,
    ...
    // Do NOT receive `unique_rec` by value. If this function fails to move out
    // the unique_ptr, we do not want this to destruct the pointer. `initialize`
    // (the caller) still relies on the pointee being alive after this call.
    // Only move out if a `capsule` is going to keep it alive.
    auto *rec = unique_rec.get();

    // Keep track of strdup'ed strings, and clean them up as long as the
    // function's capsule has not taken ownership yet (when
    // `unique_rec.release()` is called). Note: This cannot easily be fixed by a
    // `unique_ptr` with custom deleter, because the strings are only referenced
    // before strdup'ing. So only *after* the following block could `destruct`
    // safely be called, but even then, `repr` could still throw in the middle
    // of copying all strings.
    ...
    std::string signature = detail::generate_function_signature(
        text, rec, types, type_index, arg_index);
    ...
    if (rec->sibling) {
      if (PyCFunction_Check(rec->sibling.ptr())) {
        ...
        if (self == nullptr) {
          pybind11_fail(
              "initialize_generic: Unexpected nullptr from "
    ...
    if (!chain) {
      ...
      (reinterpret_cast<detail::function_record_PyObject *>(py_func_rec.ptr()))
          ->cpp_func_rec = unique_rec.release();
      guarded_strdup.release();

      object scope_module = detail::get_scope_module(rec->scope);
      m_ptr =
          PyCFunction_NewEx(rec->def, py_func_rec.ptr(), scope_module.ptr());
      if (!m_ptr) {
        pybind11_fail(
      ...
      if (chain->is_method != rec->is_method) {
        pybind11_fail(
            "overloading a method with both static and instance methods is not "
            "supported; "
#if !defined(PYBIND11_DETAILED_ERROR_MESSAGES)
            "#define PYBIND11_DETAILED_ERROR_MESSAGES or compile in debug mode "
            "for more "
            "details"
#else
      ...
      if (rec->prepend) {
        ...
        // head-of-the-chain pointer with this one, then make this one point to
        // the previous head of the chain.
        chain_start = rec;
        rec->next = chain;
        auto *py_func_rec =
            reinterpret_cast<detail::function_record_PyObject *>(
    ...
    auto *doc = signatures.empty() ? nullptr
    ...
    if (rec->is_method) {
      ...
      if (!m_ptr) {
        pybind11_fail(
            "cpp_function::cpp_function(): Could not allocate instance method "
            "object");
      }
      Py_DECREF(func);
    }
  ...
  friend void detail::function_record_PyTypeObject_methods::tp_dealloc_impl(
      PyObject *);
...
// See https://github.com/python/cpython/pull/22670
  ...
  static PyObject *dispatcher(PyObject *self, PyObject *const *args_in_arr,
                              size_t nargsf, PyObject *kwnames_in) {
    using namespace detail;
    ...
    if (overloads->is_constructor) {
      if (!parent ||
        ...
        set_error(PyExc_TypeError,
                  "__init__(self, ...) called with invalid or missing `self` "
                  "argument");
        return nullptr;
      }

      auto *const tinfo = get_type_info(
          reinterpret_cast<PyTypeObject *>(overloads->scope.ptr()));
    ...
    try {
      ...
      for (; current_overload != nullptr;
        ...
        if (n_args_in < pos_args && func.args.size() < pos_args) {
          continue;  // Not enough positional arguments given, and not enough
                     // defaults to fill in the blanks
        }
        ...
        for (; args_copied < args_to_copy; ++args_copied) {
          ...
          /* if the argument is listed in the call site's kwargs, but the
          argument is also fulfilled positionally, then the call can't match
          this overload. for example, the call site is: foo(0, key=1) but our
          overload is foo(key:int) then this call can't be for us, because it
          would be invalid.
          */
        ...
        // Keep track of how many position args we copied out in case we need to
        // come back to copy the rest into a py::args argument.
        ...
        if (args_copied < num_args) {
          for (; args_copied < num_args; ++args_copied) {
            ...
            // If we're at the py::args index then first insert a stub for it to
            // be replaced later
        ...
        // 5. Put everything in a vector.  Not technically step 5, we've been
      ...
      if (overloaded && !second_pass.empty() &&
        ...
        for (auto &call : second_pass) {
          ...
          if (result.ptr() != PYBIND11_TRY_NEXT_OVERLOAD) {
            // The error reporting logic below expects 'current_overload' to be
            // valid, as it would be if we'd encountered this failure in the
            // first-pass loop.
            if (!result) {
    ...
    if (result.ptr() == PYBIND11_TRY_NEXT_OVERLOAD) {
      ...
      for (size_t ti = overloads->is_constructor ? 1 : 0; ti < n_args_in;
           ++ti) {
        if (!some_args) {
          some_args = true;
        } else {
      ...
      if (kwnames_in && PyTuple_GET_SIZE(kwnames_in) > 0) {
        ...
        bool first = true;
        for (size_t i = 0;
             i < static_cast<size_t>(PyTuple_GET_SIZE(kwnames_in)); ++i) {
          if (first) {
            first = false;
          } else {
            msg += ", ";
  ...
  static ssize_t keyword_index(PyObject *haystack, char const *needle) {
    /* kwargs is usually very small (<= 5 entries).  The arg strings are
     * typically interned. CPython itself implements the search this way, first
     * comparing all pointers ... which is cheap and will work if the strings
     * are interned.  If it fails, then it falls back to a second lexicographic
     * check. This is wildly expensive for huge argument lists, but those are
     * incredibly rare so we optimize for the vastly common case of just a
     * couple of args.
     */
    auto n = PyTuple_GET_SIZE(haystack);
    auto s =
        reinterpret_steal<pybind11::str>(PyUnicode_InternFromString(needle));
...
class multiple_interpreters {
 public:
  enum class level {
    ...
    per_interpreter_gil  /// Use to activate
  ...
  static multiple_interpreters shared_gil() {
    return multiple_interpreters(level::shared_gil);
  }
...
/*
Return a borrowed reference to the named module if it has been successfully
initialized within this interpreter before. nullptr if it has not been
...
/*
A Py_mod_create slot function which will return the previously created module
from the cache if one exists, and otherwise will create a new module object.
*/
inline PyObject *cached_create_module(PyObject *spec, PyModuleDef *) {
  (void)&cache_completed_module;  // silence unused-function warnings, it is
                                  // used in a macro
...
/// Must be a POD type, and must hold enough entries for all of the possible
/// slots PLUS ONE for the sentinel (0) end slot.
...
inline slots_array init_slots(int (*exec_fn)(PyObject *),
                              Options &&...options) noexcept {
  /* NOTE: slots_array MUST be large enough to hold all possible options.  If
  you add an option here, you MUST also increase the size of slots_array in the
  type alias above! */
  slots_array mod_def_slots;
  size_t next_slot = 0;

  mod_def_slots[next_slot++] = {
      Py_mod_create, reinterpret_cast<void *>(&cached_create_module)};

  if (exec_fn != nullptr) {
    mod_def_slots[next_slot++] = {Py_mod_exec,
                                  reinterpret_cast<void *>(exec_fn)};
  }

#ifdef Py_mod_multiple_interpreters
  mod_def_slots[next_slot++] = {Py_mod_multiple_interpreters,
                                multi_interp_slot(options...)};
#endif
...
class module_ : public object {
 public:
  ...
  module_ def_submodule(const char *name, const char *doc = nullptr) {
    ...
    (!defined(GRAALPY_VERSION_NUM) || GRAALPY_VERSION_NUM < 0x190000)
  ...
  PYBIND11_NOINLINE void add_object(const char *name, handle obj,
    ...
    if (!overwrite && hasattr(*this, name)) {
      pybind11_fail(
          "Error during initialization: multiple incompatible definitions with "
          "name \"" +
          std::string(name) + "\"");
    }
  ...
  static module_ create_extension_module(
      const char *name, const char *doc, PyModuleDef *def,
    ...
    // TODO: Should be reinterpret_steal for Python 3, but Python also steals it
    // again when
...
class generic_type : public object {
 ...
 protected:
  void initialize(const type_record &rec) {
    ...
    tinfo->get_trampoline_self_life_support =
        rec.get_trampoline_self_life_support;
...
#endif
      internals.registered_types_py[reinterpret_cast<PyTypeObject *>(m_ptr)] = {
          tinfo};
      PYBIND11_WARNING_POP
    });
...
inline void add_class_method(object &cls, const char *name_,
...
/// Type trait to rebind a member function pointer's class to `Derived`,
/// preserving all cv/ref/noexcept qualifiers. The primary template has no
/// `type` member, providing SFINAE failure for unsupported member function
/// pointer types. `source_class` holds the original class for use in
/// `is_accessible_base_of` checks.
template <typename Derived, typename T>
struct rebind_member_ptr {};

// Define one specialization per supported qualifier combination via a local
// macro. The qualifiers argument appears in type position, not expression
// position, so parenthesizing it would produce invalid C++. The no-qualifier
// specialization is written out explicitly to avoid invoking the macro with an
...
#define PYBIND11_REBIND_MEMBER_PTR(qualifiers)                               \
  template <typename Derived, typename Return, typename Class,               \
            typename... Args>                                                \
...
/// Shared implementation body for all method_adaptor member-function-pointer
/// overloads. Asserts Base is accessible from Derived, then casts the member
/// pointer.
template <typename Derived, typename T,
...
constexpr PYBIND11_ALWAYS_INLINE Adapted adapt_member_ptr(T pmf) {
  static_assert(detail::is_accessible_base_of<typename Traits::source_class,
                                              Derived>::value,
...
/// For all other callables (lambdas, function pointers, etc.), forward
/// unchanged.
///
/// Two overloads cover all cases without explicit per-qualifier instantiations:
///
///  (1) Generic fallback — disabled for member function pointers so that (2)
///  wins
///      without any partial-ordering ambiguity.
///  (2) MFP overload — SFINAE on rebind_member_ptr::type, which exists for
///  every
///      supported qualifier combination (const, &, &&, noexcept, ...).  A
///      single template therefore covers all combinations that
///      rebind_member_ptr handles.
template <typename /*Derived*/, typename F,
          detail::enable_if_t<!std::is_member_function_pointer<
                                  detail::remove_reference_t<F>>::value,
                              int> = 0>
...
constexpr Adapted method_adaptor(T pmf) {
  // Expected to be redundant (SFINAE on rebind_member_ptr) but cheap and makes
  // the intent explicit.
...
// The only purpose of these functions is to support .def_readonly &
// .def_readwrite. In this context, the PM template parameter is certain to be a
// Pointer to a Member. The main purpose of must_be_member_function_pointer is
// to make this obvious, and to guard against accidents. As a side-effect, it
// also explains why the syntactical overhead for perfect forwarding is not
// needed.
template <typename PM>
using must_be_member_function_pointer =
    enable_if_t<std::is_member_pointer<PM>::value, int>;

// Note that property_cpp_function is intentionally in the main pybind11
// namespace, because user-defined specializations could be useful.

// Classic (non-smart_holder) implementations for .def_readonly and
// .def_readwrite getter and setter functions. WARNING: This classic
// implementation can lead to dangling pointers for raw pointer members. See
// test_ptr() in tests/test_class_sh_property.py However, this implementation
// works as-is (and safely) for smart_holder std::shared_ptr members.
template <typename T, typename D>
struct property_cpp_function_classic {
  template <typename PM, must_be_member_function_pointer<PM> = 0>
  static cpp_function readonly(PM pm, const handle &hdl) {
    return cpp_function([pm](const T &c) -> const D & { return c.*pm; },
                        is_method(hdl));
  }
  ...
  static cpp_function write(PM pm, const handle &hdl) {
    return cpp_function([pm](T &c, const D &value) { c.*pm = value; },
                        is_method(hdl));
  }
};
...
struct both_t_and_d_use_type_caster_base<
    ...
    enable_if_t<all_of<std::is_base_of<type_caster_base<T>, type_caster<T>>,
                       std::is_base_of<type_caster_base<intrinsic_t<D>>,
                                       make_caster<D>>>::value>>
    : std::true_type {};

// Specialization for raw pointer members, using smart_holder if that is the
// class_ holder, or falling back to the classic implementation if not. WARNING:
// Like the classic implementation, this implementation can lead to dangling
// pointers. See test_ptr() in tests/test_class_sh_property.py However, the read
// functions return a shared_ptr to the member, emulating the PyCLIF approach:
...
struct property_cpp_function_sh_raw_ptr_member {
    ...
    if (tinfo->holder_enum_v == holder_enum_t::smart_holder) {
      return cpp_function(
          [pm](handle c_hdl) -> std::shared_ptr<drp> {
            std::shared_ptr<T> c_sp = type_caster<
  ...
  static cpp_function write(PM pm, const handle &hdl) {
    ...
    if (tinfo->holder_enum_v == holder_enum_t::smart_holder) {
      return cpp_function(
          [pm](T &c, D value) { c.*pm = std::forward<D>(std::move(value)); },
          is_method(hdl));
    }
...
// Specialization for members held by-value, using smart_holder if that is the
// class_ holder, or falling back to the classic implementation if not. The read
// functions return a shared_ptr to the member, emulating the PyCLIF approach:
// https://github.com/google/clif/blob/c371a6d4b28d25d53a16e6d2a6d97305fb1be25a/clif/python/instance.h#L233
    ...
    if (tinfo->holder_enum_v == holder_enum_t::smart_holder) {
      return cpp_function(
          [pm](handle c_hdl)
            ...
            std::shared_ptr<T> c_sp = type_caster<
                std::shared_ptr<T>>::shared_ptr_with_responsible_parent(c_hdl);
            return std::shared_ptr<typename std::add_const<D>::type>(
                c_sp, &(c_sp.get()->*pm));
          },
          is_method(hdl));
    }
...
// Specialization for std::unique_ptr members, using smart_holder if that is the
// class_ holder, or falling back to the classic implementation if not. read
// disowns the member unique_ptr. write disowns the passed Python object.
// readonly is disabled (static_assert) because there is no safe & intuitive way
// to make the member accessible as a Python object without disowning the member
// unique_ptr. A .def_readonly disowning the unique_ptr member is deemed highly
...
struct property_cpp_function<
    T, D,
    detail::enable_if_t<
        detail::all_of<std::is_pointer<D>,
...
struct property_cpp_function<
    ...
    detail::enable_if_t<detail::all_of<
        detail::none_of<std::is_pointer<D>, std::is_array<D>,
...
template <typename T, typename D>
struct property_cpp_function<T, D,
                             detail::enable_if_t<detail::all_of<
                                 detail::is_instantiation<std::unique_ptr, D>,
                                 detail::both_t_and_d_use_type_caster_base<
                                     T, typename D::element_type>>::value>>
...
//       Running the pybind11 unit tests with smart_holder as the default holder
//       is to ensure that `py::smart_holder` / `py::classh` is
//       backward-compatible with all pre-existing functionality. Be careful not
//       to link translation units compiled with different default holders,
//       because this will cause ODR violations
...
class class_ : public detail::generic_type {
 ...
 public:
  ...
  using holder_type =
      detail::exactly_one_t<is_holder, default_holder_type<type>, options...>;
  ...
  static_assert(
      !has_alias || !detail::is_smart_holder<holder_type>::value ||
  ...
  static_assert(
      !has_alias || detail::is_smart_holder<holder_type>::value ||
          !std::is_base_of<trampoline_self_life_support, type_alias>::value,
      "pybind11::trampoline_self_life_support is a smart_holder feature, "
      "therefore"
  ...
  class_(handle scope, const char *name, const Extra &...extra) {
    ...
    if (std::is_base_of<trampoline_self_life_support, type_alias>::value) {
      ...
      // class_<type, alias_type>, but it can be called safely from any other
      // DSO.
    ...
    if (has_alias) {
      with_internals([&](internals &internals) {
        ...
        if (record.module_local) {
          ...
          internals.registered_types_cpp[std::type_index(typeid(type_alias))] =
              val;
#if PYBIND11_INTERNALS_VERSION >= 12
  ...
  static void add_base(detail::type_record &rec) {
    ...
    if PYBIND11_MAYBE_CONSTEXPR (!detail::is_static_downcastable<Base,
                                                                 type>::value) {
  ...
  // Intentionally no &&/const&& overloads: buffer protocol callbacks are
  // invoked on an existing Python object and should not move-from self.
  template <typename Return, typename Class, typename... Args>
  class_ &def_buffer(Return (Class::*func)(Args...) &) {
    return def_buffer([func](type &obj) { return (obj.*func)(); });
  }
  ...
  class_ &def_readwrite(const char *name, D C::*pm, const Extra &...extra) {
    ...
    def_property(name, property_cpp_function<type, D>::read(pm, *this),
  ...
  class_ &def_readwrite_static(const char *name, D *pm, const Extra &...extra) {
    cpp_function fget([pm](const object &) -> const D & { return *pm; },
                      scope(*this)),
        fset([pm](const object &, const D &value) { *pm = value; },
             scope(*this));
  ...
  class_ &def_property_static(const char *name, const cpp_function &fget,
    ...
    static_assert(
        0 == detail::constexpr_sum(detail::is_call_guard<Extra>::value...),
        "def_property family does not currently support call_guard. Use a "
        "py::cpp_function instead.");
    static_assert(
        0 == detail::constexpr_sum(detail::is_keep_alive<Extra>::value...),
    ...
    if (rec_fget) {
      ...
      // Args added by process_attributes (e.g. "self" via is_method +
      // pos_only/kw_only) need their strings strdup'd: initialize_generic's
      // strdup loop already ran during cpp_function construction, so it won't
      // process these late additions. Without this, destruct() would call
      // free() on string literals. See gh-5976.
      for (auto i = args_before; i < rec_fget->args.size(); ++i) {
        if (rec_fget->args[i].name) {
          rec_fget->args[i].name =
              PYBIND11_COMPAT_STRDUP(rec_fget->args[i].name);
        }
        if (rec_fget->args[i].descr) {
          rec_fget->args[i].descr =
              PYBIND11_COMPAT_STRDUP(rec_fget->args[i].descr);
        }
      }
    }
    if (rec_fset) {
      ...
      for (auto i = args_before; i < rec_fset->args.size(); ++i) {
        if (rec_fset->args[i].name) {
          rec_fset->args[i].name =
              PYBIND11_COMPAT_STRDUP(rec_fset->args[i].name);
        }
        if (rec_fset->args[i].descr) {
          rec_fset->args[i].descr =
              PYBIND11_COMPAT_STRDUP(rec_fset->args[i].descr);
        }
      }
      if (!rec_active) {
        rec_active = rec_fset;
 ...
 private:
  ...
  static void init_holder_from_existing(
      const detail::value_and_holder &v_h, const holder_type *holder_ptr,
  ...
  /// object, if possible
  static void init_holder(
      detail::instance *inst, detail::value_and_holder &v_h,
  ...
  static bool try_initialization_using_shared_from_this(holder_type *,
                                                        WrappedType *, ...) {
    return false;
  }

  // Adopting existing approach used by type_caster_base, although it leads to
  // somewhat fuzzy ownership semantics: if we detected via shared_from_this
  // that a shared_ptr exists already, it is reused, irrespective of the
  // return_value_policy in effect. "SomeBaseOfWrappedType" is needed because
  // std::enable_shared_from_this is not necessarily a direct base of
  // WrappedType.
  template <typename WrappedType, typename SomeBaseOfWrappedType>
  static bool try_initialization_using_shared_from_this(
      holder_type *uninitialized_location, WrappedType *value_ptr_w_t,
    ...
    new (uninitialized_location)
        holder_type(holder_type::from_shared_ptr(shd_ptr));
    return true;
  }
  ...
  static void init_instance(detail::instance *inst,
    ...
    // Need for const_cast is a consequence of the type_info::init_instance
    // type: void (*init_instance)(instance *, const void *);
    ...
    inst->is_alias = detail::dynamic_raw_ptr_cast_if_possible<type_alias>(
                         value_ptr_w_t) != nullptr;
    if (holder_void_ptr) {
      // Note: inst->owned ignored.
  ...
  // Deallocates an instance; via holder, if constructed; otherwise via operator
  // delete. NOTE: The Python error indicator needs to cleared BEFORE this
  // function is called. This is because we could be deallocating while cleaning
  // up after a Python exception. If the error indicator is not cleared but the
  // C++ destructor code makes Python C API calls, those calls are likely to
  // generate a new exception, and pybind11 will then throw `error_already_set`
  // from the C++ destructor. This is forbidden and will trigger
  // std::terminate().
  ...
  static void dealloc_release_gil_before_calling_cpp_dtor(
...
// Supports easier switching between py::class_<T> and py::class_<T,
// py::smart_holder>: users can simply replace the `_` in `class_` with `h` or
// vice versa.
template <typename type_, typename... options>
...
struct enum_base {
  ...
  PYBIND11_NOINLINE void init(bool is_arithmetic, bool is_convertible) {
    ...
    auto static_property = handle(
        reinterpret_cast<PyObject *>(get_internals().static_property_type));
    ...
    m_base.attr("__repr__") = cpp_function(
        ...
        name("__repr__"), is_method(m_base), pos_only());

    m_base.attr("name") = property(
        cpp_function(&enum_name, name("name"), is_method(m_base), pos_only()));
    ...
    m_base.attr("__str__") = cpp_function(
        ...
        name("__str__"), is_method(m_base), pos_only());
...
#define PYBIND11_ENUM_OP_STRICT(op, expr, strict_behavior)          \
  m_base.attr(op) = cpp_function(                                   \
      ...
      name(op), is_method(m_base), arg("other"), pos_only())
      ...
      if (is_arithmetic) {
        ...
        m_base.attr("__invert__") =
            cpp_function([](const object &arg) { return ~(int_(arg)); },
                         name("__invert__"), is_method(m_base), pos_only());
      }
    } else {
...
#undef PYBIND11_ENUM_OP_STRICT
    ...
    m_base.attr("__getstate__") =
        cpp_function([](const object &arg) { return int_(arg); },
                     name("__getstate__"), is_method(m_base), pos_only());

    m_base.attr("__hash__") =
        cpp_function([](const object &arg) { return int_(arg); },
                     name("__hash__"), is_method(m_base), pos_only());
  }
...
using equivalent_integer_t =
    typename equivalent_integer<std::is_signed<IntLike>::value,
                                sizeof(IntLike)>::type;
...
class enum_ : public class_<Type> {
 public:
  ...
  using Scalar = detail::conditional_t<
      detail::any_of<detail::is_std_char_type<Underlying>,
  ...
  enum_(const handle &scope, const char *name, const Extra &...extra)
    ...
    constexpr bool is_convertible =
        std::is_convertible<Type, Underlying>::value;
    m_base.init(is_arithmetic, is_convertible);

    def(init([](Scalar i) { return static_cast<Type>(i); }), arg("value"));
    def_property_readonly(
        "value", [](Type value) { return (Scalar)value; }, pos_only());
    def(
        "__int__", [](Type value) { return (Scalar)value; }, pos_only());
    def(
        "__index__", [](Type value) { return (Scalar)value; }, pos_only());
    attr("__setstate__") = cpp_function(
        ...
        is_method(*this), arg("state"), pos_only());
  }
...
PYBIND11_NOINLINE void keep_alive_impl(size_t Nurse, size_t Patient,
  ...
  if (res.second) {
    ...
    weakref(reinterpret_cast<PyObject *>(type), cpp_function([type](handle wr) {
              with_internals([type](internals &internals) {
                ...
                // TODO consolidate the erasure code in pybind11_meta_dealloc()
                // in class.h
...
iterator make_iterator_impl(Iterator first, Sentinel last, Extra &&...extra) {
  using state = detail::iterator_state<Access, Policy, Iterator, Sentinel,
                                       ValueType, Extra...>;
  // TODO: state captures only the types of Extra, not the values

  // For Python < 3.14.0rc1, pycritical_section uses direct mutex locking (same
  // as a unique lock), which may deadlock during type registration. See
  // detail/internals.h for details.
  ...
  if (!detail::get_type_info(typeid(state), false)) {
    class_<state>(handle(), "iterator", pybind11::module_local())
        ...
        .def(
            ...
            std::forward<Extra>(extra)..., pos_only(), Policy);
  }

  return cast(
      state{std::forward<Iterator>(first), std::forward<Sentinel>(last), true});
}
...
template <return_value_policy Policy = return_value_policy::reference_internal,
          ...
          typename... Extra>
// NOLINTNEXTLINE(performance-unnecessary-value-param)
typing::Iterator<ValueType> make_iterator(Iterator first, Sentinel last,
                                          Extra &&...extra) {
...
template <return_value_policy Policy = return_value_policy::reference_internal,
          ...
          typename... Extra>
typing::Iterator<KeyType> make_key_iterator(Iterator first, Sentinel last,
                                            Extra &&...extra) {
...
void implicitly_convertible() {
  ...
  auto implicit_caster = [](PyObject *obj, PyTypeObject *type) -> PyObject * {
    ...
    args[0] = obj;
    PyObject *result =
        PyObject_Call(reinterpret_cast<PyObject *>(type), args.ptr(), nullptr);
    if (result == nullptr) {
      PyErr_Clear();
    }
    return result;
...
inline void register_exception_translator(ExceptionTranslator &&translator) {
  detail::with_exception_translators(
      [&](std::forward_list<ExceptionTranslator> &exception_translators,
        ...
        exception_translators.push_front(
 ...
 * the exception.
 */
inline void register_local_exception_translator(
    ExceptionTranslator &&translator) {
  detail::with_exception_translators(
      [&](std::forward_list<ExceptionTranslator> &exception_translators,
          std::forward_list<ExceptionTranslator> &local_exception_translators) {
        (void)exception_translators;
        local_exception_translators.push_front(
            std::forward<ExceptionTranslator>(translator));
      });
}
...
exception<CppException> &register_exception_impl(handle scope, const char *name,
                                                 handle base, bool isLocal) {
  PYBIND11_CONSTINIT static gil_safe_call_once_and_store<
 ...
 */
template <typename CppException>
exception<CppException> &register_local_exception(
    handle scope, const char *name, handle base = PyExc_Exception) {
  return detail::register_exception_impl<CppException>(scope, name, base,
                                                       true /* isLocal */);
}
...
inline void error_already_set::m_fetched_error_deleter(
    detail::error_fetch_and_normalize *raw_ptr) {
...
inline function get_type_override(const void *this_ptr,
  ...
  if (result == nullptr) throw error_already_set();
  Py_DECREF(result);
  if (d["self"].is_none()) return function();
#endif
...
#define PYBIND11_OVERRIDE_IMPL(ret_type, cname, name, ...)                    \
  do {                                                                        \
    ...
    if (override) {                                                           \
      ...
      PYBIND11_WARNING_PUSH                                                   \
      PYBIND11_WARNING_DISABLE_MSVC(4127)                                     \
      if PYBIND11_MAYBE_CONSTEXPR (pybind11::detail::                         \
                                       cast_is_temporary_value_reference<     \
                                           ret_type>::value &&                \
                                   !pybind11::detail::is_same_ignoring_cvref< \
                                       ret_type, PyObject *>::value) {        \
...
#define PYBIND11_OVERLOAD_NAME(ret_type, cname, name, fn, ...)                \
  PYBIND11_OVERRIDE_NAME(PYBIND11_TYPE(ret_type), PYBIND11_TYPE(cname), name, \
                         fn, __VA_ARGS__)
#define PYBIND11_OVERLOAD_PURE_NAME(ret_type, cname, name, fn, ...)          \
  PYBIND11_OVERRIDE_PURE_NAME(PYBIND11_TYPE(ret_type), PYBIND11_TYPE(cname), \
                              name, fn, __VA_ARGS__);
#define PYBIND11_OVERLOAD(ret_type, cname, fn, ...)                    \
  PYBIND11_OVERRIDE(PYBIND11_TYPE(ret_type), PYBIND11_TYPE(cname), fn, \
                    __VA_ARGS__)
