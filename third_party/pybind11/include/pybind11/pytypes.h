...
class object_api : public pyobject_tag {
 ...
 public:
  ...
  /** \rst
       Similar to the above attr functions with the difference that the
  templated Type is used to set the `__annotations__` dict value to the
  corresponding key. Worth noting that attr_with_type_hint is implemented in
  cast.h. \endrst */
  template <typename T>
...
using is_pyobj_ptr_or_nullptr_t =
    detail::any_of<std::is_same<T, PyObject *>,
...
class handle : public detail::object_api<handle> {
 ...
 private:
  ...
  void throw_gilstate_error(const std::string &function_name) const {
    fprintf(
        stderr,
        "%s is being called while the GIL is either not held or invalid. "
        "Please see "
        "https://pybind11.readthedocs.io/en/stable/advanced/"
        "misc.html#common-sources-of-global-interpreter-lock-errors for "
        "debugging advice.\n"
        "If you are convinced there is no bug in your code, you can #define "
        "PYBIND11_NO_ASSERT_GIL_HELD_INCREF_DECREF "
        "to disable this check. In that case you have to ensure this #define "
        "is consistently "
        "used for all translation units linked into a given pybind11 "
        "extension, otherwise "
...
struct error_fetch_and_normalize {
  ...
  std::string format_value_and_trace() const {
    ...
    if (m_value) {
      ...
      if (!value_str) {
        ...
        // Not using `value_str.cast<std::string>()`, to not potentially throw a
        // secondary error_already_set that will then result in process
        // termination (#4288).
        auto value_bytes = reinterpret_steal<object>(PyUnicode_AsEncodedString(
        ...
        if (!value_bytes) {
          ...
          char *buffer = nullptr;
          Py_ssize_t length = 0;
          if (PyBytes_AsStringAndSize(value_bytes.ptr(), &buffer, &length) ==
              -1) {
            message_error_string = detail::error_string();
            result = message_unavailable_exc;
          } else {
            ...
            if (!note_bytes) {
              ...
              if (PyBytes_AsStringAndSize(note_bytes.ptr(), &buffer, &length) ==
                ...
                result += "\nFAILURE formatting __notes__[" +
                          std::to_string(i) + "]: " + detail::error_string();
              } else {
                result += '\n';
                result += std::string(buffer, static_cast<std::size_t>(length));
              }
  ...
  void restore() {
    if (m_restore_called) {
      pybind11_fail(
          "Internal error: "
          "pybind11::detail::error_fetch_and_normalize::restore() "
...
inline std::string error_string() {
  return error_fetch_and_normalize("pybind11::detail::error_string")
      .error_string();
}
...
class PYBIND11_EXPORT_EXCEPTION error_already_set : public std::exception {
 public:
  ...
  /// An alternate version of `discard_as_unraisable()`, where a string provides
  /// information on the location of the error. For example, `__func__` could be
  /// helpful. WARNING: The GIL must be held when this member function is
  /// called!
 ...
 private:
  ...
  ///          crashes (undefined behavior) if the Python interpreter is
  ///          finalizing.
  static void m_fetched_error_deleter(
...
inline void raise_from(error_already_set &err, PyObject *type,
                       const char *message) {
  err.restore();
...
inline PyObject *dict_setdefaultstring(PyObject *v, const char *key,
                                       PyObject *defaultobj) {
...
// See also:
// https://github.com/python/pythoncapi-compat/blob/main/pythoncapi_compat.h
inline PyObject *dict_setdefaultstringref(PyObject *v, const char *key,
                                          PyObject *defaultobj) {
...
PYBIND11_WARNING_DISABLE_MSVC(
    4522)  // warning C4522: multiple assignment operators specified
template <typename Policy>
class accessor : public object_api<accessor<Policy>> {
 ...
 public:
  ...
  template <typename T>
  enable_if_t<!std::is_same<accessor, remove_reference_t<T>>::value> operator=(
      T &&value) && {
  ...
  PYBIND11_DEPRECATED(
      "Use of obj.attr(...) as bool is deprecated in favor of "
...
struct list_item {
  ...
  static void set(handle obj, const IdxType &index, handle val) {
    ...
    if (PyList_SetItem(obj.ptr(), ssize_t_cast(index), val.inc_ref().ptr()) !=
        0) {
      throw error_already_set();
    }
  }
...
inline bool PyStaticMethod_Check(PyObject *o) {
  return Py_TYPE(o) == &PyStaticMethod_Type;
}
...
#define PYBIND11_OBJECT_COMMON(Name, Parent, CheckFun)                      \
 public:                                                                    \
  ...
  template <                                                                \
      typename Policy_> /* NOLINTNEXTLINE(google-explicit-constructor) */   \
...
class str : public object {
 public:
  ...
  template <typename T, detail::enable_if_t<
                            std::is_same<T, std::string_view>::value, int> = 0>
  // NOLINTNEXTLINE(google-explicit-constructor)
  ...
  \endrst */
  // Templatized to avoid ambiguity with str(const object&) for object-derived
  // types.
  template <typename T,
            detail::enable_if_t<
                !std::is_base_of<object, detail::remove_cvref_t<T>>::value &&
                    std::is_constructible<handle, T>::value,
                int> = 0>
  explicit str(T &&h)
      : object(raw_str(handle(std::forward<T>(h)).ptr()), stolen_t{}) {
    if (!m_ptr) {
      throw error_already_set();
    }
  }
...
Unsigned as_unsigned(PyObject *o) {
  if (sizeof(Unsigned) <= sizeof(unsigned long)) {
    ...
    return v == static_cast<unsigned long>(-1) && PyErr_Occurred()
               ? (Unsigned)-1
               : (Unsigned)v;
  }
  unsigned long long v = PyLong_AsUnsignedLongLong(o);
  return v == static_cast<unsigned long long>(-1) && PyErr_Occurred()
             ? (Unsigned)-1
             : (Unsigned)v;
}
PYBIND11_NAMESPACE_END(detail)
...
class float_ : public object {
 public:
  ...
  // NOLINTNEXTLINE(google-explicit-constructor)
  float_(float value)
      : object(PyFloat_FromDouble(static_cast<double>(value)), stolen_t{}) {
    if (!m_ptr) {
...
class capsule : public object {
 public:
  ...
  /// Replaces a capsule's name *without* calling the destructor on the existing
  /// one.
 ...
 private:
  ...
  void initialize_with_void_ptr_destructor(const void *value, const char *name,
    ...
    if (!m_ptr || PyCapsule_SetContext(
                      m_ptr, reinterpret_cast<void *>(destructor)) != 0) {
      throw error_already_set();
    }
  }
};
...
class dict : public object {
 public:
  ...
  template <
      ...
      // MSVC workaround: it can't compile an out-of-line definition, so defer
      // the collector
      typename collector =
...
class memoryview : public object {
 public:
  ...
  static memoryview from_buffer(const void *ptr, ssize_t itemsize,
                                const char *format,
                                detail::any_container<ssize_t> shape,
                                detail::any_container<ssize_t> strides) {
  ...
  /** \rst
   ...
   \endrst */
  static memoryview from_memory(void *mem, ssize_t size,
                                bool readonly = false) {
    PyObject *ptr =
...
#define PYBIND11_MATH_OPERATOR_UNARY(op, fn)                        \
  ...
  object object_api<D>::op() const {                                \
    ...
    if (!result.ptr()) throw error_already_set();                   \
    return result;                                                  \
  }
