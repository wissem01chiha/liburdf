...
class native_enum : public detail::native_enum_data {
 public:
  ...
  native_enum(handle parent_scope, const char *name,
              const char *native_type_name, const char *class_doc = "")
      : detail::native_enum_data(parent_scope, name, native_type_name,
                                 class_doc, make_record()) {
    if (detail::get_local_type_info(typeid(EnumType)) != nullptr ||
      ...
      pybind11_fail("pybind11::native_enum<...>(\"" + enum_name_encoded +
                    "\") is already registered as a `pybind11::enum_` or "
                    "`pybind11::class_`!");
    }
  ...
  native_enum &value(char const *name, EnumType value,
                     const char *doc = nullptr) {
    // Disarm for the case that the native_enum_data dtor runs during exception
    // unwinding.
