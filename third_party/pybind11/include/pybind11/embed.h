...
inline void initialize_interpreter_pre_pyconfig(bool init_signal_handlers,
  ...
  std::vector<std::unique_ptr<wchar_t[], detail::wide_char_arg_deleter>>
...
/** \rst
 ...
 https://docs.python.org/3/c-api/init.html#c.PySys_SetArgvEx \endrst */
inline void initialize_interpreter(bool init_signal_handlers = true,
                                   int argc = 0,
                                   const char *const *argv = nullptr,
                                   bool add_program_dir_to_path = true) {
#if PY_VERSION_HEX < PYBIND11_PYCONFIG_SUPPORT_PY_VERSION_HEX
  detail::initialize_interpreter_pre_pyconfig(init_signal_handlers, argc, argv,
...
inline void finalize_interpreter() {
  ...
  // Re-fetch the internals pointer-to-pointer (but not the internals itself,
  // which might not exist). It's possible for the  internals to be created
  // during Py_Finalize() (e.g. if a py::capsule calls `get_internals()` during
  // destruction), so we get the pointer-pointer here and check it after
  // Py_Finalize().
