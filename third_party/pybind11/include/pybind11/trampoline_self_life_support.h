...
struct trampoline_self_life_support {
  ...
  // For the next two, the default implementations generate undefined behavior
  // (ASAN failures manually verified). The reason is that v_h needs to be kept
  // default-initialized.
  ...
  // These should never be needed (please provide test cases if you think they
  // are).
  trampoline_self_life_support &operator=(
      const trampoline_self_life_support &) = delete;
  trampoline_self_life_support &operator=(trampoline_self_life_support &&) =
      delete;
};

PYBIND11_NAMESPACE_BEGIN(detail)
using get_trampoline_self_life_support_fn =
    trampoline_self_life_support *(*)(void *);
PYBIND11_NAMESPACE_END(detail)
