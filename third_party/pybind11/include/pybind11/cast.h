...
// type_caster to a T
template <typename T>
typename make_caster<T>::template cast_op_type<T> cast_op(
    make_caster<T> &caster) {
  using result_t =
      typename make_caster<T>::template cast_op_type<T>;  // See PR #4893
...
class type_caster_enum_type {
 ...
 public:
  ...
  static handle cast(SrcType &&src, return_value_policy, handle parent) {
    handle native_enum = global_internals_native_enum_type_map_get_item(
    ...
    return type_caster_base<EnumType>::cast(
        ...
        // Fixes
        // https://github.com/pybind/pybind11/pull/3643#issuecomment-1022987818:
        return_value_policy::copy, parent);
  }
  ...
  bool load(handle src, bool convert) {
    ...
    if (native_enum) {
      ...
      native_value =
          static_cast<EnumType>(static_cast<Underlying>(underlying_caster));
      native_loaded = true;
      return true;
    }
...
class type_caster<std::reference_wrapper<type>> {
 private:
  ...
  static_assert(
      std::is_same<typename std::remove_const<type>::type &,
...
class type_caster<void> : public type_caster<void_type> {
 public:
  ...
  bool load(handle h, bool) {
    ...
    const auto &bases = all_type_info(
        reinterpret_cast<PyTypeObject *>(type::handle_of(h).ptr()));
...
class type_caster<bool> {
 ...
 private:
  ...
  static bool is_numpy_bool(handle object) {
    ...
    // Name changed to `numpy.bool` in NumPy 2, `numpy.bool_` is needed for 1.x
    // support
...
struct string_caster {
  ...
  bool load(handle src, bool) {
    ...
    size_t length = static_cast<size_t>(PYBIND11_BYTES_SIZE(utfNbytes.ptr())) /
                    sizeof(CharT);
 ...
 private:
  static handle decode_utfN(const char *buffer, ssize_t nbytes) {
    ...
    // PyUnicode_DecodeUTF32 as well), so bypass the whole thing by just passing
    // the encoding as a string value, which works properly:
  ...
  // sizes this is a no-op. which supports loading a unicode from a str, doesn't
  // take this path.
 ...
 protected:
  ...
  static handle cast_impl(T &&src, return_value_policy policy, handle parent,
    ...
    std::array<object, size> entries{{reinterpret_steal<object>(
        ...
        make_caster<Ts>::cast(std::get<Is>(std::forward<T>(src)), policy,
                              parent))...}};
...
// SMART_HOLDER_BAKEIN_FOLLOW_ON: Rewrite comment, with reference to shared_ptr
// specialization.
...
struct copyable_holder_caster : public type_caster_base<type> {
 ...
 protected:
  ...
  bool set_foreign_holder(handle src) {
    return holder_caster_foreign_helpers::set_foreign_holder(src, (type *)value,
                                                             &holder);
  }
...
// SMART_HOLDER_BAKEIN_FOLLOW_ON: Refactor copyable_holder_caster to reduce code
// duplication.
template <typename type>
struct copyable_holder_caster<
 ...
 public:
  ...
  bool load(handle src, bool convert) {
    if (base::template load_impl<
            copyable_holder_caster<type, std::shared_ptr<type>>>(src,
                                                                 convert)) {
  ...
  std::weak_ptr<type> potentially_slicing_weak_ptr() {
    if (sh_load_helper.was_populated) {
      ...
      shared_ptr_storage = sh_load_helper.load_as_shared_ptr(
          typeinfo, value,
  ...
  static handle cast(const std::shared_ptr<type> &src,
                     return_value_policy policy, handle parent) {
  ...
  // This function will succeed even if the `responsible_parent` does not own
  // the wrapped C++ object directly. It is the responsibility of the caller to
  // ensure that the `responsible_parent` has a `keep_alive` relationship with
  // the owner of the wrapped C++ object, or that the wrapped C++ object lives
  // for the duration of the process.
  static std::shared_ptr<type> shared_ptr_with_responsible_parent(
      handle responsible_parent) {
    copyable_holder_caster loader;
    loader.load(responsible_parent, /*convert=*/false);
    assert(loader.typeinfo->holder_enum_v ==
           detail::holder_enum_t::smart_holder);
 ...
 protected:
    ...
    for (auto &cast : typeinfo->implicit_casts) {
      ...
      if (sub_caster.load(src, convert)) {
        ...
        if (typeinfo->holder_enum_v == detail::holder_enum_t::smart_holder) {
          ...
          shared_ptr_storage = std::shared_ptr<type>(
              sub_caster.shared_ptr_storage, (type *)value);
        }
        return true;
      }
    }
  ...
  smart_holder_type_caster_support::load_helper<remove_cv_t<type>>
      sh_load_helper;  // Const2Mutbl
...
/// Return a std::shared_ptr with the SAME CONTROL BLOCK as the std::shared_ptr
/// owned by the class_ holder. For class_-wrapped types with trampolines, the
/// returned std::shared_ptr does NOT keep any derived Python objects alive (see
/// issue #1333).
///
/// For class_-wrapped types using std::shared_ptr as the holder, the following
/// expressions produce equivalent results (see
/// tests/test_potentially_slicing_weak_ptr.cpp,py):
///
///     - obj.cast<std::shared_ptr<T>>()
///     - py::potentially_slicing_weak_ptr<T>(obj).lock()
///
/// For class_-wrapped types with trampolines and using py::smart_holder,
/// obj.cast<>() produces a std::shared_ptr that keeps any derived Python
/// objects alive for its own lifetime, but this is achieved by introducing a
/// std::shared_ptr control block that is independent of the one owned by the
/// py::smart_holder. This can lead to surprising std::weak_ptr behavior (see
/// issue #5623). An easy solution is to use
/// py::potentially_slicing_weak_ptr<>(obj), as exercised in
/// tests/test_potentially_slicing_weak_ptr.cpp,py (look for
/// "set_wp_potentially_slicing"). Note, however, that this reintroduces the
/// inheritance slicing issue (see issue #1333). The ideal — but usually more
/// involved — solution is to use a Python weakref to the derived Python object,
/// instead of a C++ base-class std::weak_ptr.
///
/// It is not possible (at least no known approach exists at the time of this
/// writing) to simultaneously achieve both desirable properties:
///
///     - the same std::shared_ptr control block as the class_ holder
///     - automatic lifetime extension of any derived Python objects
///
/// The reason is that this would introduce a reference cycle that cannot be
/// garbage collected:
///
///     - the derived Python object owns the class_ holder
///     - the class_ holder owns the std::shared_ptr
///     - the std::shared_ptr would own a reference to the derived Python
///     object,
...
std::weak_ptr<T> potentially_slicing_weak_ptr(handle obj) {
  ...
  throw type_error(
      ...
      "\" object is not convertible to std::weak_ptr<T> (with T = " +
      type_id<T>() + ")");
}

PYBIND11_NAMESPACE_BEGIN(detail)

// SMART_HOLDER_BAKEIN_FOLLOW_ON: Rewrite comment, with reference to unique_ptr
// specialization.
...
// SMART_HOLDER_BAKEIN_FOLLOW_ON: Refactor move_only_holder_caster to reduce
// code duplication.
template <typename type, typename deleter>
struct move_only_holder_caster<
    type, std::unique_ptr<type, deleter>,
    enable_if_t<
 ...
 public:
  ...
  static handle cast(std::unique_ptr<type, deleter> &&src,
                     return_value_policy policy, handle parent) {
    auto *ptr = src.get();
    typename type_caster_base<type>::cast_sources srcs{ptr};
    if (srcs.creates_smart_holder()) {
  ...
  static handle cast(const std::unique_ptr<type, deleter> &src,
                     return_value_policy policy, handle parent) {
    if (!src) {
      return none().release();
    }
  ...
  bool set_foreign_holder(handle) {
    throw cast_error(
        "Foreign instance cannot be converted to std::unique_ptr "
  ...
  using cast_op_type = conditional_t<
      ...
      const std::unique_ptr<type, deleter> &, std::unique_ptr<type, deleter>>;

  explicit operator std::unique_ptr<type, deleter>() {
    if (typeinfo->holder_enum_v == detail::holder_enum_t::smart_holder) {
      return sh_load_helper.template load_as_unique_ptr<deleter>(typeinfo,
                                                                 value);
    }
  ...
  explicit operator const std::unique_ptr<type, deleter> &() {
    if (typeinfo->holder_enum_v == detail::holder_enum_t::smart_holder) {
      // Get shared_ptr to ensure that the Python object is not disowned
      // elsewhere.
      shared_ptr_storage = sh_load_helper.load_as_shared_ptr(typeinfo, value);
      ...
      unique_ptr_storage = std::shared_ptr<std::unique_ptr<type, deleter>>(
          ...
          [](std::unique_ptr<type, deleter> *ptr) {
            if (!ptr) {
              pybind11_fail(
                  "FATAL: `const std::unique_ptr<T, D> &` was disowned "
...
#define PYBIND11_DECLARE_HOLDER_TYPE(type, holder_type, ...)         \
  ...
  struct always_construct_holder<holder_type>                        \
      : always_construct_holder_value<__VA_ARGS__> {};               \
  template <typename type>                                           \
...
struct handle_type_name<KWArgs<T>> {
  static constexpr auto name = io_name("**kwargs: ", "dict[str, ") +
                               make_caster<T>::name + io_name("", "]");
};
template <>
...
struct pyobject_caster {
  ...
  // Initializing `value` to a nil handle is safe since it will only be accessed
  // if `load` succeeds.
  template <typename T = type,
...
template <typename T, detail::enable_if_t<!detail::is_pyobject<T>::value &&
                                              !detail::is_same_ignoring_cvref<
                                                  T, PyObject *>::value,
                                          int> = 0>
T cast(const handle &handle) {
  using namespace detail;
  constexpr bool is_enum_cast =
      type_uses_type_caster_enum_type<intrinsic_t<T>>::value;
  static_assert(
      !cast_is_temporary_value_reference<T>::value || is_enum_cast,
...
detail::enable_if_t<!detail::move_never<T>::value, T> move(object &&obj) {
  if (obj.ref_count() > 1) {
#if !defined(PYBIND11_DETAILED_ERROR_MESSAGES)
    throw cast_error("Unable to cast Python " +
...
template <typename T>
detail::enable_if_t<
    !detail::is_pyobject<T>::value && detail::move_always<T>::value, T>
cast(object &&object) {
  return move<T>(std::move(object));
}
template <typename T>
detail::enable_if_t<
    !detail::is_pyobject<T>::value && detail::move_if_unreferenced<T>::value, T>
cast(object &&object) {
...
template <typename T>
detail::enable_if_t<
    !detail::is_pyobject<T>::value && detail::move_never<T>::value, T>
cast(object &&object) {
  return cast<T>(object);
}
...
str_attr_accessor object_api<D>::attr_with_type_hint(const char *key) const {
  ...
  if (ann.contains(key)) {
    throw std::runtime_error("__annotations__[\"" + std::string(key) +
                             "\"] was set already.");
  }
...
// this is a no-op.
template <typename T>
enable_if_t<cast_is_temporary_value_reference<T>::value, T> cast_ref(
    object &&o, make_caster<T> &caster) {
...
inline cast_error cast_error_unable_to_convert_call_arg(
  ...
  return cast_error("Unable to convert call argument '" + name + "' of type '" +
                    type + "' to Python object");
}
#endif
...
/// is the equivalent of an unnamed '/' argument
struct pos_only {};
...
struct function_call {
  function_call(const function_record &f,
                handle p);  // Implementation in attr.h
 ...
 public:
  ...
  explicit unpacking_collector(Ts &&...values)
      : m_names(reinterpret_steal<tuple>(
            handle()))  // initialize to null to avoid useless allocation of
                        // 0-length tuple
  {
    /*
    Python can sometimes utilize an extra space before the arguments to prepend
    `self`. This is important enough that there is a special flag for it:
    PY_VECTORCALL_ARGUMENTS_OFFSET.
    All we have to do is allocate an extra space at the beginning of this array,
    and set the flag. Note that the extra space is not passed directly in to
    vectorcall.
    */
    ...
    if (args_has_keyword_or_ds<Ts...>()) {
      ...
      // collect_arguments guarantees this can't be constructed with kwargs
      // before the last positional so we don't need to worry about Ts... being
      // in anything but normal python order.
      using expander = int[];
      (void)expander{0, (process(names_list, std::forward<Ts>(values)), 0)...};

      m_names = reinterpret_steal<tuple>(PyList_AsTuple(names_list.ptr()));
    } else {
      auto not_used = reinterpret_steal<list>(
          handle());  // initialize as null (to avoid an allocation)
  ...
  object call(PyObject *ptr) const {
    size_t nargs =
        m_args.size() - 1;  // -1 for PY_VECTORCALL_ARGUMENTS_OFFSET (see ctor)
    if (m_names) {
      nargs -= m_names.size();
    }
    PyObject *result =
...
#endif
            ptr, m_args.data() + 1, nargs | PY_VECTORCALL_ARGUMENTS_OFFSET,
            m_names.ptr());
    if (!result) {
      throw error_already_set();
    }
  ...
  void process(list & /*names_list*/, T &&x) {
    ...
    if (!h) {
      ...
      throw cast_error_unable_to_convert_call_arg(
          std::to_string(m_args.size() - 1));
#else
      throw cast_error_unable_to_convert_call_arg(
          std::to_string(m_args.size() - 1), type_id<T>());
#endif
    }
...
handle type::handle_of() {
  static_assert(std::is_base_of<detail::type_caster_generic,
                                detail::make_caster<T>>::value,
                "py::type::of<T> only supports the case where T is a "
                "registered C++ types.");
