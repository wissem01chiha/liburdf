...
struct func_wrapper : func_wrapper_base {
  ...
  Return operator()(
      Args... args) const {  // NOLINT(performance-unnecessary-value-param)
    gil_scoped_acquire acq;
 ...
 public:
  bool load(handle src, bool convert) {
    ...
    if (auto cfunc = func.cpp_function()) {
      ...
      // Raising an fail exception here works to prevent the segfault, but only
      // on gcc. See PR #1413 for full details
    }

    value =
        type_caster_std_function_specializations::func_wrapper<Return, Args...>(
            type_caster_std_function_specializations::func_handle(
                std::move(func)));
    return true;
  }
