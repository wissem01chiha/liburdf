      ...
      [](Vector &v, const slice &slice, const Vector &value) {
        ...
        if (slicelength != value.size()) {
          throw std::runtime_error(
              "Left and right hand size of slice assignment have different "
              "sizes!");
        }
...
// std::vector
//
template <typename Vector, typename holder_type = default_holder_type<Vector>,
          typename... Args>
...
void map_assignment(
    enable_if_t<is_copy_assignable<typename Map::mapped_type>::value, Class_>
        &cl) {
...
inline str format_message_key_error_key_object(handle py_key) {
  ...
  if (len(message) > 2 * cut_length + 3) {
    return str(message[slice(0, cut_length, 1)]) + str("✄✄✄") +
           str(message[slice(-cut_length, static_cast<ssize_t>(len(message)),
                             1)]);
  }
  return message;
}
...
template <typename Map, typename holder_type = default_holder_type<Map>,
          typename... Args>
