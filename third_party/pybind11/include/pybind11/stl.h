...
Additional checks have been added to allow types derived from
`collections.abc.Set` and `collections.abc.Mapping` (`collections.abc.Sequence`
is already allowed by `PySequence_Check`).
*/

inline bool object_is_instance_with_one_of_tp_names(
...
inline bool object_is_convertible_to_std_vector(const handle &src) {
  ...
  return (PyGen_Check(src.ptr()) != 0) || (PyAnySet_Check(src.ptr()) != 0) ||
         object_is_instance_with_one_of_tp_names(
             src.ptr(),
             {"dict_keys", "dict_values", "dict_items", "map", "zip"});
}
...
inline bool object_is_convertible_to_std_set(const handle &src, bool convert) {
  ...
  return ((PyAnySet_Check(src.ptr()) != 0) ||
         ...
         (convert &&
          isinstance(src, module_::import("collections.abc").attr("Set")));
}
...
inline bool object_is_convertible_to_std_map(const handle &src, bool convert) {
  ...
  // According to https://docs.python.org/3/c-api/mapping.html,
  // `PyMappingCheck()` checks for
  // `__getitem__()` without checking the type of keys. In order to restrict the
  // allowed types closer to actual Mapping-like types, we also check for the
  // `items()` method.
  ...
  return convert &&
         isinstance(src, module_::import("collections.abc").attr("Mapping"));
}
...
struct set_caster {
 ...
 public:
  ...
  PYBIND11_TYPE_CASTER(type, io_name("collections.abc.Set", "set") +
                                 const_name("[") + key_conv::name +
                                 const_name("]"));
};
...
struct map_caster {
 ...
 public:
  ...
  PYBIND11_TYPE_CASTER(Type, io_name("collections.abc.Mapping", "dict") +
                                 const_name("[") + key_conv::name +
...
struct list_caster {
  ...
  bool load(handle src, bool convert) {
    ...
    // The conversion to a tuple will first exhaust the generator object, to
    // ensure that the generator is not left in an unpredictable (to the caller)
 ...
 public:
  ...
  PYBIND11_TYPE_CASTER(Type, io_name("collections.abc.Sequence", "list") +
                                 const_name("[") + value_conv::name +
                                 const_name("]"));
};
...
ArrayType vector_to_array(V &&v) {
  return vector_to_array_impl<ArrayType, V>(std::forward<V>(v),
...
struct array_caster {
 ...
 private:
  ...
  bool convert_elements(handle seq, bool convert) {
    ...
    value.reset(
        new ArrayType(vector_to_array<ArrayType, Size>(std::move(temp))));
    return true;
  }
 ...
 public:
  ...
  template <
      typename T_,
      enable_if_t<std::is_same<ArrayType, remove_cv_t<T_>>::value, int> = 0>
...
struct variant_caster<V<Ts...>> {
  ...
  PYBIND11_TYPE_CASTER(
      Type, ::pybind11::detail::union_concat(make_caster<Ts>::name...));
};
