...
inline bool PyWarning_Check(PyObject *obj) {
  ...
  if (result == -1) {
    raise_from(PyExc_SystemError,
               "pybind11::detail::PyWarning_Check(): PyObject_IsSubclass() "
               "call failed.");
...
inline object new_warning_type(handle scope, const char *name,
                               handle base = PyExc_RuntimeWarning) {
  if (!detail::PyWarning_Check(base.ptr())) {
    pybind11_fail(
        "pybind11::warnings::new_warning_type(): cannot create custom warning, "
        "base "
  ...
  if (hasattr(scope, name)) {
    pybind11_fail(
        "pybind11::warnings::new_warning_type(): an attribute with name \"" +
  ...
  if (!h) {
    raise_from(PyExc_SystemError,
               "pybind11::warnings::new_warning_type(): PyErr_NewException() "
               "call failed.");
    throw error_already_set();
  }
...
inline void warn(const char *message, handle category = PyExc_RuntimeWarning,
                 int stack_level = 2) {
  if (!detail::PyWarning_Check(category.ptr())) {
    pybind11_fail(
        "pybind11::warnings::warn(): cannot raise warning, category must be a "
        "subclass of "
        "PyExc_Warning!");
  }
