...
// Tuple type hint defined in cast.h for use in py::make_tuple to avoid circular
// includes
...
struct handle_type_name<typing::Callable<Return(Args...)>> {
  ...
  static constexpr auto name =
      ...
      ::pybind11::detail::concat(
          ::pybind11::detail::arg_descr(make_caster<Args>::name)...) +
      const_name("], ") +
...
struct handle_type_name<typing::Union<Types...>> {
  static constexpr auto name =
      ::pybind11::detail::union_concat(make_caster<Types>::name...);
};
...
struct handle_type_name<typing::Final<T>> {
  static constexpr auto name =
      const_name("typing.Final[") +
      ::pybind11::detail::return_descr(make_caster<T>::name) + const_name("]");
};
...
struct handle_type_name<typing::TypeGuard<T>> {
  static constexpr auto name = const_name(PYBIND11_TYPE_GUARD_TYPE_HINT) +
                               const_name("[") + make_caster<T>::name +
                               const_name("]");
};

template <typename T>
struct handle_type_name<typing::TypeIs<T>> {
  static constexpr auto name = const_name(PYBIND11_TYPE_IS_TYPE_HINT) +
                               const_name("[") + make_caster<T>::name +
                               const_name("]");
};
...
consteval auto sanitize_string_literal() {
  ...
  constexpr auto num_special_chars =
      std::accumulate(special_chars.begin(), special_chars.end(), (size_t)0,
                      [&v](auto acc, const char &c) {
...
struct handle_type_name<typing::Literal<Literals...>> {
  static constexpr auto name =
      ...
      pybind11::detail::concat(
          const_name(sanitize_string_literal<Literals>().name)...) +
      const_name("]");
};
template <typing::StringLiteral StrLit>
struct handle_type_name<typing::TypeVar<StrLit>> {
  static constexpr auto name =
      const_name(sanitize_string_literal<StrLit>().name);
};
#endif
