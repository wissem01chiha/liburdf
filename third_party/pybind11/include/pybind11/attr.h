...
struct base {
  PYBIND11_DEPRECATED(
      "base<T>() was deprecated in favor of specifying 'T' as a template "
      "argument to class_")
  base() = default;
};
...
/// Annotation which enables releasing the GIL before calling the C++ destructor
/// of wrapped instances (pybind/pybind11#1446).
...
struct op_;
void keep_alive_impl(size_t Nurse, size_t Patient, function_call &call,
                     handle ret);
...
struct argument_record {
  ...
  bool
      convert : 1;  ///< True if the argument is allowed to convert when loading
...
#define PYBIND11_DETAIL_FUNCTION_RECORD_ABI_ID \
  "v1"  // PLEASE UPDATE if the struct is changed.
struct function_record {
  ...
  /// Number of leading positional arguments, which are terminated by a py::args
  /// or py::kwargs argument or by a py::kw_only annotation.
...
// The main purpose of this macro is to make it easy to pin-point the critically
// related code sections.
#define PYBIND11_ENSURE_PRECONDITION_FOR_FUNCTIONAL_H_PERFORMANCE_OPTIMIZATIONS( \
    ...)                                                                         \
  static_assert(__VA_ARGS__,                                                     \
                "Violation of precondition for pybind11/functional.h "           \
...
struct type_record {
  ...
  PYBIND11_NOINLINE void add_base(const std::type_info &base,
    ...
    bool this_has_unique_ptr_holder =
        (holder_enum_v == holder_enum_t::std_unique_ptr);
...
#else
    dynamic_attr |=
        (PyType_GetFlags(base_info->type) & Py_TPFLAGS_MANAGED_DICT) != 0;
#endif
...
inline void check_kw_only_arg(const arg &a, function_record *r) {
  if (r->args.size() > r->nargs_pos && (!a.name || a.name[0] == '\0')) {
    pybind11_fail(
        "arg(): cannot specify an unnamed argument after a kw_only() "
        "annotation or "
        "args() argument");
  }
}
...
struct process_attribute<kw_only> : process_attribute_default<kw_only> {
  static void init(const kw_only &, function_record *r) {
    ...
    if (r->has_args &&
      ...
      pybind11_fail(
          "Mismatched args() and kw_only(): they must occur at the same "
          "relative "
...
struct process_attribute<pos_only> : process_attribute_default<pos_only> {
  static void init(const pos_only &, function_record *r) {
    ...
    // It also can't follow a kw_only, but a static_assert in pybind11.h checks
    // that
  }
};
...
struct process_attributes {
  static void init(const Args &...args, function_record *r) {
    ...
    using expander = int[];
    (void)expander{
        0, ((void)process_attribute<typename std::decay<Args>::type>::init(args,
                                                                           r),
            0)...};
  }
...
constexpr bool expected_num_args(size_t nargs, bool has_args, bool has_kwargs) {
  ...
  return named == 0 || (self + named + static_cast<size_t>(has_args) +
                        static_cast<size_t>(has_kwargs)) == nargs;
}
