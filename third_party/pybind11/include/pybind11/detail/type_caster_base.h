...
class loader_life_support {
 private:
  // Thread-local top-of-stack for loader_life_support frames (linked via
  // parent). Observation: loader_life_support needs to be thread-local, but we
  // don't need to go to extra effort to keep it per-interpreter (i.e., by
  // putting it in internals) since individual function calls are already
  // isolated to a single interpreter, even though they could potentially call
  // into a different interpreter later in the same call chain.  This saves a
  // significant cost per function call spent in loader_life_support
  // destruction. Note for future C++17 simplification: inline static
  // thread_local loader_life_support *tls_current_frame = nullptr;
 ...
 public:
  ...
  PYBIND11_NOINLINE static void add_patient(handle h) {
    ...
    if (!frame) {
      ...
      // indicates use of pybind11::cast<> outside the normal call framework,
      // finding such a location is challenging. Developers could consider
      // printing out stack frame addresses here using something like
...
// fashion. See PR #4762.
inline void all_type_info_add_base_most_derived_first(
    std::vector<type_info *> &bases, type_info *addl_base) {
 ...
 */
inline const std::vector<detail::type_info *> &all_type_info(
    PyTypeObject *type) {
...
PYBIND11_NOINLINE detail::type_info *get_type_info(PyTypeObject *type) {
  ...
  if (bases.size() > 1) {
    pybind11_fail(
        "pybind11::detail::get_type_info: type has multiple "
...
inline detail::type_info *get_global_type_info_lock_held(
    const std::type_info &tp) {
...
PYBIND11_NOINLINE detail::type_info *get_type_info(
    const std::type_info &tp, bool throw_if_missing = false) {
  PYBIND11_LOCK_INTERNALS(get_internals());
...
PYBIND11_NOINLINE handle get_type_handle(const std::type_info &tp,
  ...
  return handle(type_info ? (reinterpret_cast<PyObject *>(type_info->type))
                          : nullptr);
}
    ...
    iterator(instance *inst, const type_vec *tinfo) : inst{inst}, types{tinfo} {
      if (inst != nullptr) {
        ...
        curr = value_and_holder(
            inst /* instance */, (*types)[0] /* type info */,
...
PYBIND11_NOINLINE void instance::allocate_layout() {
  ...
  if (n_types == 0) {
    pybind11_fail(
        "instance allocation failed: new instance has no pybind11-registered "
        "base types");
  }
  ...
  if (simple_layout) {
    ...
    // allocation functions: Python is using pymalloc, which is designed to be
    // efficient for small allocations like the one we're doing here;
    // for larger allocations they are just wrappers around malloc.
    // TODO: is this still true for pure Python 3.6?
    nonsimple.values_and_holders =
        static_cast<void **>(PyMem_Calloc(space, sizeof(void *)));
...
// Information about how type_caster_generic::cast() can obtain its source
// object
struct cast_sources {
...
struct value_and_holder_helper {
  ...
  void throw_if_uninitialized_or_disowned_holder(
  ...
  void throw_if_instance_is_currently_owned_by_shared_ptr(
      const type_info *tinfo) const {
...
handle smart_holder_from_unique_ptr(std::unique_ptr<T, D> &&src,
  ...
  // cs.cppobj is the subobject pointer appropriate for tinfo (may differ from
  // src.get() under MI/VI). Use this for Python identity/registration, but keep
  // ownership on T*.
  void *src_raw_void_ptr = const_cast<void *>(cs.cppobj);
  assert(cs.tinfo != nullptr);
  const detail::type_info *tinfo = cs.tinfo;
  if (handle existing_inst =
          find_registered_python_instance(src_raw_void_ptr, tinfo)) {
    ...
    if (self_life_support != nullptr) {
      ...
      if (v_h.inst != nullptr && v_h.vh != nullptr) {
        ...
        if (!holder.is_disowned) {
          pybind11_fail(
              "smart_holder_from_unique_ptr: unexpected "
  ...
  if (static_cast<void *>(src.get()) == src_raw_void_ptr) {
    // This is a multiple-inheritance situation that is incompatible with the
    // current shared_from_this handling (see PR #3023). Is there a better
    // solution?
...
handle smart_holder_from_shared_ptr(const std::shared_ptr<T> &src,
  ...
  switch (policy) {
    ...
    case return_value_policy::take_ownership:
      throw cast_error(
          "Invalid return_value_policy for shared_ptr (take_ownership).");
    case return_value_policy::copy:
    case return_value_policy::move:
      break;
    case return_value_policy::reference:
      throw cast_error(
          "Invalid return_value_policy for shared_ptr (reference).");
  ...
  if (handle existing_inst =
          find_registered_python_instance(src_raw_void_ptr, tinfo)) {
    // PYBIND11:REMINDER: MISSING: Enforcement of consistency with existing
    // smart_holder. PYBIND11:REMINDER: MISSING: keep_alive.
    return existing_inst;
  }
  ...
  auto smhldr = smart_holder::from_shared_ptr(
      std::shared_ptr<void>(src, src_raw_void_ptr));
  tinfo->init_instance(inst_raw_ptr, static_cast<const void *>(&smhldr));
...
handle smart_holder_from_shared_ptr(const std::shared_ptr<T const> &src,
                                    return_value_policy policy, handle parent,
                                    const cast_sources::resolved_source &cs) {
  return smart_holder_from_shared_ptr(
...
template <typename T, typename D,
          typename std::enable_if<std::is_default_constructible<D>::value,
                                  int>::type = 0>
inline std::unique_ptr<T, D> unique_with_deleter(T *raw_ptr,
                                                 std::unique_ptr<D> &&deleter) {
  if (deleter == nullptr) {
...
inline std::unique_ptr<T, D> unique_with_deleter(T *raw_ptr,
  ...
  if (deleter == nullptr) {
    pybind11_fail(
        "smart_holder_type_casters: deleter is not default constructible and no"
...
struct load_helper : value_and_holder_helper {
  ...
  void maybe_set_python_instance_is_alias(handle src) {
    if (was_populated) {
      python_instance_is_alias =
          reinterpret_cast<instance *>(src.ptr())->is_alias;
    }
  }

  static std::shared_ptr<T> make_shared_ptr_with_responsible_parent(
      T *raw_ptr, handle parent) {
    return std::shared_ptr<T>(raw_ptr,
                              shared_ptr_parent_life_support(parent.ptr()));
  }

  std::shared_ptr<T> load_as_shared_ptr(
      const type_info *tinfo, void *void_raw_ptr,
    ...
    if (hld.vptr_is_using_noop_deleter) {
      if (responsible_parent) {
        return make_shared_ptr_with_responsible_parent(
            static_cast<T *>(void_raw_ptr), responsible_parent);
      }
    ...
    if (python_instance_is_alias && !force_potentially_slicing_shared_ptr) {
      ...
      if (vptr_gd_ptr != nullptr) {
        ...
        std::shared_ptr<T> to_be_released(
            type_raw_ptr,
            shared_ptr_trampoline_self_life_support(loaded_v_h.inst));
        vptr_gd_ptr->released_ptr = to_be_released;
        return to_be_released;
      }
      auto *sptsls_ptr =
          std::get_deleter<shared_ptr_trampoline_self_life_support>(hld.vptr);
      if (sptsls_ptr != nullptr) {
        // This code is reachable only if there are multiple
        // registered_instances for the same pointee.
        if (reinterpret_cast<PyObject *>(loaded_v_h.inst) == sptsls_ptr->self) {
          pybind11_fail(
              "smart_holder_type_caster_support load_as_shared_ptr failure: "
              "loaded_v_h.inst == sptsls_ptr->self");
        }
      }
      if (sptsls_ptr != nullptr ||
          !memory::type_has_shared_from_this(type_raw_ptr)) {
      ...
      if (hld.vptr_is_external_shared_ptr) {
        pybind11_fail(
            "smart_holder_type_casters load_as_shared_ptr failure: not "
            "implemented: trampoline-self-life-support for external shared_ptr "
            "to type inheriting from std::enable_shared_from_this.");
      }
      pybind11_fail(
          "smart_holder_type_casters: load_as_shared_ptr failure: internal "
          "inconsistency.");
    }
  ...
  template <typename D>
  std::unique_ptr<T, D> load_as_unique_ptr(
      const type_info *tinfo, void *raw_void_ptr,
    ...
    // This is enforced indirectly by a static_assert in the class_
    // implementation:
    assert(!python_instance_is_alias || self_life_support);

    std::unique_ptr<D> extracted_deleter =
        holder().template extract_deleter<T, D>(
    ...
    auto result =
        unique_with_deleter<T, D>(raw_type_ptr, std::move(extracted_deleter));
    if (self_life_support != nullptr) {
  ...
  // This assumes load_as_shared_ptr succeeded(), and the returned shared_ptr is
  // still alive. The returned unique_ptr is meant to never expire (the behavior
  // is undefined otherwise).
  template <typename D>
  std::unique_ptr<T, D> load_as_const_unique_ptr(
      const type_info *tinfo, T *raw_type_ptr,
    ...
    return unique_with_deleter<T, D>(
        raw_type_ptr, std::move(holder().template extract_deleter<T, D>(
                          context, tinfo->get_memory_guarded_delete)));
  }
};
...
class type_caster_generic {
 public:
  ...
  static handle cast(const void *src, return_value_policy policy, handle parent,
  ...
  PYBIND11_NOINLINE static handle cast(const cast_sources &srcs,
    ...
    if (!srcs.result.tinfo) {
      ...
      std::string tname = srcs.downcast.cpptype ? srcs.downcast.cpptype->name()
                          : srcs.original.cpptype
                              ? srcs.original.cpptype->name()
                              : "<unspecified>";
      detail::clean_type_id(tname);
...
inline object cpp_conduit_method(handle self,
  ...
  if (std::strcmp(cpp_type_info_capsule.name(),
                  typeid(std::type_info).name()) != 0) {
    return none();
  }
...
struct container_mapped_type_traits<
    Container, typename std::enable_if<std::is_same<
                   typename Container::mapped_type, Container>::value>::type> {
...
template <typename Container>
struct container_mapped_type_traits<
    Container, typename std::enable_if<negation<std::is_same<
                   typename Container::mapped_type, Container>>::value>::type> {
  static constexpr bool has_mapped_type = true;
  static constexpr bool has_recursive_mapped_type = false;
};
...
struct container_value_type_traits<
    Container, typename std::enable_if<std::is_same<
                   typename Container::value_type, Container>::value>::type> {
...
template <typename Container>
struct impl_recursive_container_traits<
    Container, typename std::enable_if<container_value_type_traits<
                   Container>::has_value_type>::type> {
  static constexpr bool is_recursive =
      container_mapped_type_traits<Container>::has_recursive_mapped_type ||
      container_value_type_traits<Container>::has_recursive_value_type;
  /*
   * This member dictates which type Pybind11 should check recursively in traits
   * such as `is_move_constructible`, `is_copy_constructible`,
   * `is_move_assignable`, ... Direct access to `value_type` should be avoided:
  ...
  using type_to_check_recursively = typename std::conditional<
      ...
      typename impl_type_to_check_recursively<
          ...
          container_mapped_type_traits<Container>::has_mapped_type>::
          if_recursive,
      typename impl_type_to_check_recursively<
          typename Container::value_type,
          container_mapped_type_traits<Container>::has_mapped_type>::
          if_not_recursive>::type;
};
...
struct is_move_constructible
    : all_of<std::is_move_constructible<T>,
             is_move_constructible<typename recursive_container_traits<
                 T>::type_to_check_recursively>> {};
...
struct is_copy_constructible
    : all_of<std::is_copy_constructible<T>,
             is_copy_constructible<typename recursive_container_traits<
                 T>::type_to_check_recursively>> {};
...
struct is_copy_assignable
    : all_of<std::is_copy_assignable<T>,
             is_copy_assignable<typename recursive_container_traits<
                 T>::type_to_check_recursively>> {};
...
class type_caster_base : public type_caster_generic {
 ...
 public:
  ...
  static handle cast(const cast_sources &srcs, return_value_policy policy,
                     handle parent) {
    return type_caster_generic::cast(
        srcs, policy, parent, make_copy_constructor((const itype *)nullptr),
  ...
  static handle cast_holder(const cast_sources &srcs, const void *holder) {
    ...
    return type_caster_generic::cast(srcs, policy, {}, nullptr, nullptr,
                                     holder);
  }
 ...
 protected:
  ...
  /* Only enabled when the types are {copy,move}-constructible *and* when the
     type does not have a private operator new implementation. A comma operator
     is used in the decltype argument to apply SFINAE to the public copy/move
     constructors.*/
