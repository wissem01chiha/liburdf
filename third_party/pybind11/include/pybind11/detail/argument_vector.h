 ...
 private:
  ...
  PYBIND11_NOINLINE void move_to_heap_vector_with_reserved_size(
    ...
    static_assert(std::is_nothrow_move_constructible<heap_vector>::value,
                  "this conversion is not exception safe");
    std::move(ha.arr.begin(), ha.arr.begin() + ha.size,
              std::back_inserter(hv.vec));
    new (&m_repr.hvector) heap_vector(std::move(hv));
  }

  PYBIND11_NOINLINE void reserve_slow_path(std::size_t sz) {
...
// Container to avoid heap allocation for kRequestedInlineSize or fewer
// booleans.
...
struct small_vector<bool, kRequestedInlineSize> {
 ...
 private:
  ...
  bool inline_index(std::size_t idx) const {
    ...
    assert(wbi.word < kWords);
    assert(wbi.bit < kBitsPerWord);
    return m_repr.iarray.arr[wbi.word] &
           (static_cast<std::size_t>(1) << wbi.bit);
  }
  ...
  PYBIND11_NOINLINE void push_back_slow_path(bool b) {
    m_repr.hvector.vec.push_back(b);
  }

  static constexpr auto kBitsPerWord = 8 * sizeof(std::size_t);
  static constexpr auto kWords =
      (kRequestedInlineSize + kBitsPerWord - 1) / kBitsPerWord;
...
/// A small_vector of PyObject* that holds references and releases them on
/// destruction. This provides explicit ownership semantics without relying on
/// py::object's destructor, and avoids the need for reinterpret_cast when
/// passing to vectorcall.
...
class ref_small_vector {
 public:
  ...
  // Move is allowed
  ref_small_vector(ref_small_vector &&other) noexcept
