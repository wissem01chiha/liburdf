...
#error \
    "PYTHON < 3.8 IS UNSUPPORTED. pybind11 v2.13 was the last to support Python 3.7."
#endif

// Similar to Python's convention:
// https://docs.python.org/3/c-api/apiabiversion.html See also:
// https://github.com/python/cpython/blob/HEAD/Include/patchlevel.h
/* -- start version constants -- */
#define PYBIND11_VERSION_MAJOR 3
#define PYBIND11_VERSION_MINOR 0
#define PYBIND11_VERSION_MICRO 3
// ALPHA = 0xA, BETA = 0xB, GAMMA = 0xC (release candidate), FINAL = 0xF (stable
// release)
...
#define PYBIND11_VERSION_HEX                                                   \
  Py_PACK_FULL_VERSION(PYBIND11_VERSION_MAJOR, PYBIND11_VERSION_MINOR,         \
                       PYBIND11_VERSION_MICRO, PYBIND11_VERSION_RELEASE_LEVEL, \
...
// to simplify backward compatibility handling for users (e.g., via #ifdef
// checks):
...
#define PYBIND11_HAS_EXPERIMENTAL_FILESYSTEM 1
    ...
    !(defined(_MSC_VER) && (_MSC_VER < 1920))  // See PR #5968
...
// IMPORTANT: This code block must stay BELOW the #include <exception> above
// (see PR #5390).
...
#if defined(PYBIND11_CPP20) && defined(__cpp_lib_span) && \
...
// Slightly faster code paths are available when
// PYBIND11_HAS_SUBINTERPRETER_SUPPORT is *not* defined, so avoid defining it
// for implementations that do not support subinterpreters. However, defining it
// unnecessarily is not expected to break anything. This can be overridden by
// the user with -DPYBIND11_HAS_SUBINTERPRETER_SUPPORT=1 or 0
...
#else
#define PYBIND11_PYCFUNCTION_GET_DOC(func) \
...
Note that this is run once for each (sub-)interpreter the module is imported
into, including possibly concurrently.  The PyModuleDef is allowed to be static,
but the PyObject* resulting from PyModuleDef_Init should be treated like any
other PyObject (so not shared across interpreters).
 */
...
#define PYBIND11_MODULE_EXEC(name, variable)                                \
  ...
  int PYBIND11_CONCAT(pybind11_exec_, name)(PyObject * pm) {                \
    try {                                                                   \
      ...
      if (!pybind11::detail::get_cached_module(                             \
              m.attr("__spec__").attr("name"))) {                           \
...
code.
    ...
    The third and subsequent macro arguments are optional (available
since 2.13.0), and can be used to mark the extension module as supporting
...
struct instance {
  ...
  bool has_patients : 1;
  /// If true, this Python object needs to be kept alive for the lifetime of the
  /// C++ value.
  bool is_alias : 1;
...
// instead of
//     static_assert(false, "...");
// to trigger the static_assert() in a template only if it is actually
// instantiated.
template <typename>
...
//
// We build up a sequence S by repeatedly doubling its length and sometimes
// adding 1 to the end. E.g. if the current S is 0...3, then we either go to
// 0...7 or 0...8 on the next pass. The goal is to end with S = 0...N-1. The key
// insight is that the times we need to add an additional digit to S correspond
// exactly to the 1's in the binary representation of the number N.
//
// Invariants:
// - digit is a power of 2
// - N_digit_is_1 is whether N's binary representation has a 1 in that digit's
// position.
// - end <= N
// - S is 0...end-1.
...
template <size_t digit, bool N_digit_is_1, size_t N, size_t end,
          size_t... S>  // N_digit_is_1=false
struct make_index_sequence_impl
    : make_index_sequence_impl<digit / 2, (N & (digit / 2)) != 0, N, 2 * end,
                               S..., (S + end)...> {};
template <size_t digit, size_t N, size_t end, size_t... S>
struct make_index_sequence_impl<digit, true, N, end, S...>
    : make_index_sequence_impl<digit / 2, (N & (digit / 2)) != 0, N,
                               2 * end + 1, S..., (S + end)..., 2 * end> {};
...
/// Detects whether static_cast<Derived*>(Base*) is valid, i.e. the inheritance
/// is non-virtual. Used to detect virtual bases: if this is false, pointer
/// adjustments require the implicit_casts chain rather than reinterpret_cast.
template <typename Base, typename Derived, typename = void>
struct is_static_downcastable : std::false_type {};
template <typename Base, typename Derived>
struct is_static_downcastable<
    Base, Derived,
...
// Strip noexcept from a free function type (C++17: noexcept is part of the
// type).
template <typename T>
...
// Strips noexcept from the result so that factory/pickle_factory partial
// specializations (which match plain Return(Args...)) work correctly with
// noexcept callables (issue #2234).
...
PYBIND11_RUNTIME_EXCEPTION(cast_error,
                           PyExc_RuntimeError)  /// Thrown when pybind11::cast
                                                /// or handle::call fail due to
...
struct overload_cast_impl {
  ...
  // Define const/non-const member-pointer selector pairs for qualifier
  // combinations. The `qualifiers` parameter is used in type position, where
  // extra parentheses are invalid. NOLINTBEGIN(bugprone-macro-parentheses)
#define PYBIND11_OVERLOAD_CAST_MEMBER_PTR(qualifiers)                         \
  template <typename Return, typename Class>                                  \
  constexpr auto operator()(Return (Class::*pmf)(Args...) qualifiers,         \
                            std::false_type = {}) const noexcept              \
      -> decltype(pmf) {                                                      \
    return pmf;                                                               \
  }                                                                           \
  template <typename Return, typename Class>                                  \
  constexpr auto operator()(Return (Class::*pmf)(Args...) const qualifiers,   \
                            std::true_type) const noexcept -> decltype(pmf) { \
    return pmf;                                                               \
  }
...
#ifdef __cpp_noexcept_function_type
  template <typename Return>
  constexpr auto operator()(Return (*pf)(Args...) noexcept) const noexcept
      -> decltype(pf) {
    return pf;
  }
...
template <typename T>
inline static std::shared_ptr<T> try_get_shared_from_this(
    std::enable_shared_from_this<T> *holder_value_ptr) {
...
#define PYBIND11_WORKAROUND_INCORRECT_GCC_UNUSED_BUT_SET_PARAMETER(...)
    ...
    (defined(__apple_build_version__) /* AppleClang 13.0.0.13000029 was the \
     ...
     || (__clang_major__ >= 7 &&                                            \
         __clang_major__ <=                                                 \
             12) /* Clang 3, 5, 13, 14, 15 do not generate the warning. */  \
    )
...
// CPython 3.11+ provides Py_TPFLAGS_MANAGED_DICT, but PyPy3.11 does not, see PR
// #5508.
