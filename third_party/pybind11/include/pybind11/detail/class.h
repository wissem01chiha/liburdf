...
/// instance.
extern "C" inline PyObject *pybind11_static_get(PyObject *self,
                                                PyObject * /*ob*/,
                                                PyObject *cls) {
...
inline PyTypeObject *make_static_property_type() {
  ...
  setattr(reinterpret_cast<PyObject *>(type), "__module__",
          str(PYBIND11_DUMMY_MODULE_NAME));
...
extern "C" inline PyObject *pybind11_meta_call(PyObject *type, PyObject *args,
                                               PyObject *kwargs) {
...
inline void traverse_offset_bases(
    void *valueptr, const detail::type_info *tinfo, instance *self,
    bool (*f)(void * /*parentptr*/, instance * /*self*/)) {
  for (handle h : reinterpret_borrow<tuple>(tinfo->type->tp_bases)) {
    if (auto *parent_tinfo =
            get_type_info(reinterpret_cast<PyTypeObject *>(h.ptr()))) {
...
inline PyObject *make_new_instance(PyTypeObject *type) {
  ...
  // the first inherited object is a plain Python type (i.e. not derived from an
  // extension type).  Fix it.
...
inline void clear_patients(PyObject *self) {
  ...
  with_internals([&](internals &internals) {
    ...
    if (pos == internals.patients.end()) {
      pybind11_fail(
          "FATAL: Internal consistency check failed: Invalid clear_patients() "
          "call.");
    }
  ...
  for (auto &v_h : values_and_holders(instance)) {
    if (v_h) {
      ...
      if (v_h.instance_registered() &&
        ...
        pybind11_fail(
            "pybind11_object_dealloc(): Tried to deallocate unregistered "
            "instance!");
      }
...
inline PyObject *make_object_base_type(PyTypeObject *metaclass) {
  ...
  /* Danger zone: from now (and until PyType_Ready), make sure to
     ...
     turn find the newly constructed type in an invalid state) */
  auto *heap_type =
      reinterpret_cast<PyHeapTypeObject *>(metaclass->tp_alloc(metaclass, 0));
  if (!heap_type) {
...
extern "C" inline int pybind11_getbuffer(PyObject *obj, Py_buffer *view,
  ...
  // Fill in all the information, and then downgrade as requested by the caller,
  // or raise an error if that's not possible.
  ...
  if ((flags & PyBUF_C_CONTIGUOUS) == PyBUF_C_CONTIGUOUS) {
    ...
    if (PyBuffer_IsContiguous(view, 'A') == 0) {
      std::memset(view, 0, sizeof(Py_buffer));
      set_error(PyExc_BufferError,
                "Contiguous buffer requested for discontiguous storage");
      return -1;
    }
  ...
  // Set these after all checks so they don't leak out into the caller, and can
  // be automatically cleaned up on error.
  view->buf = info->ptr;
...
inline PyObject *make_new_python_type(const type_record &rec) {
  ...
  /* Danger zone: from now (and until PyType_Ready), make sure to
     ...
     garbage collector (the GC will call type_traverse(), which will in
     turn find the newly constructed type in an invalid state) */
  auto *metaclass = rec.metaclass.ptr()
                        ? reinterpret_cast<PyTypeObject *>(rec.metaclass.ptr())
