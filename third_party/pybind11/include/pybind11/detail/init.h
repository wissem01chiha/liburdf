...
template <typename Class>
[[noreturn]] void construct_alias_from_cpp(
    std::false_type /*!is_alias_constructible*/, value_and_holder &,
    Cpp<Class> &&) {
  throw type_error(
...
void construct(value_and_holder &v_h, Cpp<Class> *ptr, bool need_alias) {
  ...
  if (Class::has_alias && need_alias && !is_alias<Class>(ptr)) {
    ...
    v_h.set_instance_registered(
        true);  // Trick to prevent init_instance from registering it
...
void construct(value_and_holder &v_h, Holder<Class> holder, bool need_alias) {
  ...
  if (Class::has_alias && need_alias && !is_alias<Class>(ptr)) {
    throw type_error(
        "pybind11::init(): construction failed: returned holder-wrapped "
        "instance "
...
void construct(value_and_holder &v_h, Alias<Class> &&result, bool) {
  static_assert(is_move_constructible<Alias<Class>>::value,
...
smart_holder init_smart_holder_from_unique_ptr(std::unique_ptr<T, D> &&unq_ptr,
  ...
  void *void_ptr =
      void_cast_raw_ptr ? static_cast<void *>(unq_ptr.get()) : nullptr;
...
template <typename Class, typename D = std::default_delete<Cpp<Class>>,
          detail::enable_if_t<is_smart_holder<Holder<Class>>::value, int> = 0>
void construct(value_and_holder &v_h, std::unique_ptr<Cpp<Class>, D> &&unq_ptr,
               bool need_alias) {
  PYBIND11_WORKAROUND_INCORRECT_MSVC_C4100(need_alias);
  ...
  if (Class::has_alias && need_alias && !is_alias<Class>(ptr)) {
    throw type_error(
        "pybind11::init(): construction failed: returned std::unique_ptr "
        "pointee "
        "is not an alias instance");
  }
  // Here and below: if the new object is a trampoline, the shared_from_this
  // mechanism needs to be prevented from accessing the smart_holder vptr,
  // because it does not keep the trampoline Python object alive. For types that
  // don't inherit from enable_shared_from_this it does not matter if
  // void_cast_raw_ptr is true or false, therefore it's not necessary to also
  // inspect the type.
  auto smhldr = init_smart_holder_from_unique_ptr(
      std::move(unq_ptr),
      /*void_cast_raw_ptr*/ Class::has_alias && is_alias<Class>(ptr));
  v_h.value_ptr() = ptr;
...
void construct(value_and_holder &v_h,
  ...
  auto *ptr = unq_ptr.get();
  no_nullptr(ptr);
  auto smhldr = init_smart_holder_from_unique_ptr(std::move(unq_ptr),
                                                  /*void_cast_raw_ptr*/ true);
  v_h.value_ptr() = ptr;
  v_h.type->init_instance(v_h.inst, &smhldr);
}
...
void construct_from_shared_ptr(value_and_holder &v_h,
  ...
  if (Class::has_alias && need_alias && !is_alias<Class>(ptr)) {
    throw type_error(
        "pybind11::init(): construction failed: returned std::shared_ptr "
        "pointee "
        "is not an alias instance");
  }
  // Cast to non-const if needed, consistent with internal design
  auto smhldr = smart_holder::from_shared_ptr(
      std::const_pointer_cast<Cpp<Class>>(std::move(shd_ptr)));
...
template <typename Class,
          detail::enable_if_t<is_smart_holder<Holder<Class>>::value, int> = 0>
void construct(value_and_holder &v_h, std::shared_ptr<Cpp<Class>> &&shd_ptr,
               bool need_alias) {
  construct_from_shared_ptr<Cpp<Class>, Class>(v_h, std::move(shd_ptr),
                                               need_alias);
}
...
template <typename Class,
          detail::enable_if_t<is_smart_holder<Holder<Class>>::value, int> = 0>
void construct(value_and_holder &v_h,
               std::shared_ptr<const Cpp<Class>> &&shd_ptr, bool need_alias) {
  construct_from_shared_ptr<const Cpp<Class>, Class>(v_h, std::move(shd_ptr),
                                                     need_alias);
}

template <typename Class,
          detail::enable_if_t<is_smart_holder<Holder<Class>>::value, int> = 0>
void construct(value_and_holder &v_h, std::shared_ptr<Alias<Class>> &&shd_ptr,
               bool /*need_alias*/) {
  auto *ptr = shd_ptr.get();
  no_nullptr(ptr);
...
// Note: The 4th template parameter `void_type()` is explicitly specified to
// resolve a template ambiguity with the dual-factory specialization below when
// compiled with nvcc + GCC (see #5565). Without it, both specializations match
// equally well for the single-factory case, since the 4th parameter defaults to
// `function_signature_t<void_type(*)()>` = `void_type()`, which the
// dual-factory specialization can also decompose as `AReturn(AArgs...)` with
// `AReturn=void_type` and `AArgs={}`.
...
void setstate(value_and_holder &v_h, std::pair<T, O> &&result,
  ...
  if (PyDict_Check(d.ptr()) && PyDict_Size(d.ptr()) == 0) {
    // Skipping setattr below, to not force use of py::dynamic_attr() for Class
    // unnecessarily. See PR #2972 for details.
    return;
  }
  // Our tests never run into an unset dict, but being careful here for now (see
  // #5658)
  auto dict =
      getattr(reinterpret_cast<PyObject *>(v_h.inst), "__dict__", none());
  if (dict.is_none()) {
...
struct pickle_factory<Get, Set, RetState(Self), NewInstance(ArgState)> {
  ...
  // Subclasses are now allowed for support between type hint and generic
  // versions of types (e.g.) typing::List <--> list
  static_assert(
      std::is_same<Ret, Arg>::value || std::is_base_of<Ret, Arg>::value ||
