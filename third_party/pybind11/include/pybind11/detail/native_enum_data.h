...
inline std::string native_enum_missing_finalize_error_message(
...
class native_enum_data {
 public:
  native_enum_data(handle parent_scope_, const char *enum_name,
      ...
      : enum_name_encoded{enum_name},
        ...
        parent_scope(reinterpret_borrow<object>(parent_scope_)),
        enum_name{enum_name},
    ...
    enum_record = capsule(
        ...
        native_enum_record::attribute_name(), +[](void *record_) {
  ...
  ~native_enum_data() {
    if (finalize_needed) {
      pybind11_fail(
          native_enum_missing_finalize_error_message(enum_name_encoded));
    }
  }
#endif
...
inline handle global_internals_native_enum_type_map_get_item(
...
inline bool global_internals_native_enum_type_map_contains(
    const std::type_index &enum_type_index) {
  return with_internals([&](internals &internals) {
...
inline object import_or_getattr(
  ...
  auto curr_scope =
      reinterpret_steal<object>(PyImport_ImportModule(part.c_str()));
  if (!curr_scope) {
  ...
  while (std::getline(stream, part, '.')) {
    ...
    next_path += ".";
    next_path += part;
    auto next_scope = reinterpret_steal<object>(
        PyObject_GetAttrString(curr_scope.ptr(), part.c_str()));
    if (!next_scope) {
      error_fetch_and_normalize stored_getattr_error("getattr");
      // Try importing the next level
      next_scope =
          reinterpret_steal<object>(PyImport_ImportModule(next_path.c_str()));
      if (!next_scope) {
...
inline void native_enum_data::finalize() {
  ...
  auto py_enum_type =
      import_or_getattr(native_type_name, " (native_type_name)");
  ...
  if (hasattr(parent_scope, "__qualname__")) {
    const auto parent_qualname =
        parent_scope.attr("__qualname__").cast<std::string>();
    py_enum.attr("__qualname__") =
        str(parent_qualname + "." + enum_name.cast<std::string>());
  }
