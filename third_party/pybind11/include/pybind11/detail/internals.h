...
#error \
    "PYBIND11_INTERNALS_VERSION 11 is the minimum for all platforms for pybind11v3."
#endif
...
/// A smart-pointer-like wrapper around a thread-specific value. get/set of the
/// pointer applies to the current thread only.
template <typename T>
 ...
 public:
  thread_specific_storage() {
    ...
    if (!PYBIND11_TLS_KEY_CREATE(key_)) {
      pybind11_fail(
          "thread_specific_storage constructor: could not initialize the TSS "
          "key!");
    }
  }

  ~thread_specific_storage() {
    // This destructor is often called *after* Py_Finalize(). That *SHOULD BE*
    // fine on most platforms. The following details what happens when
    // PyThread_tss_free is called in CPython. PYBIND11_TLS_FREE is
    // PyThread_tss_free on python 3.7+. On older python, it does nothing.
    // PyThread_tss_free calls PyThread_tss_delete and PyMem_RawFree.
    // PyThread_tss_delete just calls TlsFree (on Windows) or pthread_key_delete
    // (on *NIX). Neither of those have anything to do with CPython internals.
    // PyMem_RawFree *requires* that the `key` be allocated with the CPython
    // allocator (as it is by PyThread_tss_create). However, in GraalPy (as of
    // v24.2 or older), TSS is implemented by Java and this call requires a
    // living Python interpreter.
  ...
  void set(T *val) {
    PYBIND11_TLS_REPLACE_VALUE(key_, reinterpret_cast<void *>(val));
  }
...
class pycritical_section {
 ...
 public:
  explicit pycritical_section(pymutex &m) : mutex(m) {
    // PyCriticalSection_BeginMutex was added in Python 3.15.0a1 and backported
    // to 3.14.0rc1
#if PY_VERSION_HEX >= 0x030E00C1  // 3.14.0rc1
...
struct internals {
  ...
  std::unordered_map<std::string, void *>
      shared_data;  // Custom data to be shared across
                    // extensions
...
#if PYBIND11_INTERNALS_VERSION <= 11
  thread_specific_storage<loader_life_support>
      loader_life_support_tls;  // OBSOLETE (PR #5830)
#endif
  // Unused if PYBIND11_SIMPLE_GIL_MANAGEMENT is defined:
  ...
  internals()
      : static_property_type(make_static_property_type()),
        ...
        istate(get_interpreter_state_unchecked()) {
    tstate.set(nullptr);  // See PR #5870
    registered_exception_translators.push_front(&translate_exception);
#ifdef Py_GIL_DISABLED
    // Scale proportional to the number of cores. 2x is a heuristic to reduce
    // contention. Make sure the number isn't unreasonable by limiting it to 16
    // bits (65K)
    auto num_shards = static_cast<std::uint16_t>(std::min<std::size_t>(
...
struct type_info {
  ...
  memory::get_guarded_delete_fn get_memory_guarded_delete =
      memory::get_guarded_delete;
   ...
   * A type can be simple even if it has non-simple ancestors as long as it has
   * no descendants.
   */
  bool simple_type : 1;
...
/// We use this to figure out if there are or have been multiple subinterpreters
/// active at any point. This must never go from true to false while any
/// interpreter may be running in any thread!
...
template <
    class T,
    enable_if_t<std::is_same<std::nested_exception, remove_cvref_t<T>>::value,
                int> = 0>
...
// Sentinel value for the `dtor` parameter of
// `atomic_get_or_create_in_state_dict`. Indicates no destructor was explicitly
// provided (distinct from nullptr, which means "leak").
...
//   - The storage is interpreter-dependent: different interpreters will have
//   different storage.
//     This is important when using multiple-interpreters, to avoid sharing
//     unshareable objects between interpreters.
//   - There is one storage per `key` in an interpreter and it is accessible
//   between all extensions
//     in the same interpreter.
//   - The life span of the storage is tied to the interpreter: it will be kept
//   alive until the
//     interpreter shuts down.
//
// Use test-and-set pattern with `PyDict_SetDefault` for thread-safe concurrent
// access. WARNING: There can be multiple threads creating the storage at the
// same time, while only one
//          will succeed in inserting its capsule into the dict. Therefore, the
//          deleter will be used to clean up the storage of the unused capsules.
//
// Returns: pair of (pointer to storage, bool indicating if newly created).
//          The bool follows std::map::insert convention: true = created, false
//          = existed.
// `dtor`: optional destructor called when the interpreter shuts down.
//   - If not provided: the storage will be deleted using `delete`.
//   - If nullptr: the storage will be leaked (useful for singletons that
//   outlive the interpreter).
//   - If a function: that function will be called with the capsule object.
template <typename Payload>
std::pair<Payload *, bool> atomic_get_or_create_in_state_dict(
    const char *key, void (*dtor)(PyObject *) = PYBIND11_DTOR_USE_DELETE) {
  ...
  if (capsule_obj == nullptr) {
    ...
    // Use unique_ptr for exception safety: if capsule creation throws, the
    ...
    auto new_capsule = capsule(
        ...
        //  destructor will be called on the newly created capsule at the end of
        //  this function, and we want to just release this memory.
        /*destructor=*/[](void *v) { delete static_cast<Payload *>(v); });
    // At this point, the capsule object is created successfully.
    // Release the unique_ptr and let the capsule object own the storage to
    // avoid double-free.
    ...
    //   - If key exists (another thread inserted first), returns the existing
    //   value.
    // This is thread-safe because `PyDict_SetDefault` will hold a lock on the
    // dict.
    //
    // NOTE: Here we use `PyDict_SetDefault` instead of `PyDict_SetDefaultRef`
    // because the
    //       capsule is kept alive until interpreter shutdown, so we do not need
    //       to handle incref and decref here.
    capsule_obj =
        dict_setdefaultstring(state_dict.ptr(), key, new_capsule.ptr());
    if (capsule_obj == nullptr) {
      throw error_already_set();
    }
    created = (capsule_obj == new_capsule.ptr());
    // - If key already existed, our `new_capsule` is not inserted, it will be
    // destructed when
    //   going out of scope here, and will call the destructor set above.
    // - Otherwise, our `new_capsule` is now in the dict, and it owns the
    // storage and the state
    //   dict will incref it.  We need to set the caller's destructor on it,
    //   which will be called when the interpreter shuts down.
...
class internals_pp_manager {
 public:
  ...
  static internals_pp_manager &get_instance(char const *id,
                                            on_fetch_function *on_fetch) {
    static internals_pp_manager instance(id, on_fetch);
    return instance;
  }

  /// Get the current pointer-to-pointer, allocating it if it does not already
  ...
  std::unique_ptr<InternalsType> *get_pp() {
    ...
    if (has_seen_non_main_interpreter()) {
      // Whenever the interpreter changes on the current thread we need to
      // invalidate the internals_pp so that it can be pulled from the
      // interpreter's state dict.  That is slow, so we use the current
      // PyThreadState to check if it is necessary.
      auto *tstate = get_thread_state_unchecked();
  ...
  void destroy() {
    ...
    if (has_seen_non_main_interpreter()) {
      auto *tstate = get_thread_state_unchecked();
      // this could be called without an active interpreter, just use what was
      // cached
      if (!tstate || tstate->interp == last_istate_tls()) {
    ...
    {
      std::lock_guard<std::mutex> lock(pp_set_mutex_);
      pps_have_created_content_.erase(
          internals_singleton_pp_);  // untrack deleted pp
    }
  ...
  void create_pp_content_once(std::unique_ptr<InternalsType> *const pp) {
    // Assume the GIL is held here. May call back into Python. We cannot hold
    // the lock with our mutex here. So there may be multiple threads creating
    // the content at the same time. Only one will install its content to pp
    // below. Others will be freed when going out of scope.
    auto tmp = std::unique_ptr<InternalsType>(new InternalsType());

    {
      // Lock scope must not include Python calls, which may require the GIL and
      // cause deadlocks.
      std::lock_guard<std::mutex> lock(pp_set_mutex_);
      ...
      // The content is either not yet created, or was previously destroyed via
      // pp->reset().

      // Detect re-creation of internals after destruction during interpreter
      // shutdown. If pybind11 code (e.g., tp_traverse/tp_clear calling
      // py::cast) runs after internals have been destroyed, a new empty
      // internals would be created, causing type lookup failures. See also
      // get_or_create_pp_in_state_dict() comments.
      if (pps_have_created_content_.find(pp) !=
          pps_have_created_content_.end()) {
        pybind11_fail(
            "pybind11::detail::internals_pp_manager::create_pp_content_once() "
            "FAILED: reentrant call detected while fetching pybind11 "
            "internals!");
      }
 ...
 private:
  ...
  std::unique_ptr<InternalsType> *get_or_create_pp_in_state_dict() {
    // The `unique_ptr<InternalsType>` is intentionally leaked on interpreter
    // shutdown. Once an instance is created, it will never be deleted until the
    // process exits (compare to interpreter shutdown in multiple-interpreter
    // scenarios). We cannot guarantee the destruction order of capsules in the
    // interpreter state dict on interpreter shutdown, so deleting internals too
    // early could cause undefined behavior when other pybind11 objects access
    // `get_internals()` during finalization (which would recreate empty
    // internals). See also create_pp_content_once() above. See
    // https://github.com/pybind/pybind11/pull/5958#discussion_r2717645230.
    auto result =
    ...
    // Only call on_fetch_ when fetching existing internals, not when creating
    // new ones.
  ...
  // Pointer-to-pointer to the singleton internals for the first seen
  // interpreter (may not be the main interpreter)
  std::unique_ptr<InternalsType> *internals_singleton_pp_ = nullptr;

  // Track pointer-to-pointers whose internals have been created, to detect
  // re-entrancy. Use instance member over static due to singleton pattern of
  // this class.
  std::unordered_set<std::unique_ptr<InternalsType> *>
      pps_have_created_content_;
  std::mutex pp_set_mutex_;
};

// If We loaded the internals through `state_dict`, our `error_already_set`
// and `builtin_exception` may be different local classes than the ones set up
// in the initial exception translator, below, so add another for our local
// exception classes.
//
...
#if !defined(__GLIBCXX__)
inline void check_internals_local_exception_translator(
...
#endif
  return internals_pp_manager<internals>::get_instance(PYBIND11_INTERNALS_ID,
                                                       ON_FETCH_FN);
#undef ON_FETCH_FN
}
...
PYBIND11_NOINLINE internals &get_internals() {
  ...
  if (!internals_ptr) {
    ...
    if (!internals_ptr->instance_base) {
      ...
      // constructor called above because internals_ptr must be set before
      // get_internals is called again
...
/// This function ensures a consistent key is used across all call sites within
/// the same compilation unit. The key includes the address of a static variable
/// to make it unique per module (DSO), matching the behavior of
/// get_local_internals_pp_manager().
...
inline internals_pp_manager<local_internals> &get_local_internals_pp_manager() {
  // Use the address of a static variable as part of the key, so that the value
  // is uniquely tied to where the module is loaded in memory
  return internals_pp_manager<local_internals>::get_instance(
      get_local_internals_key().c_str(), nullptr);
}
...
#define PYBIND11_LOCK_INTERNALS(internals) \
  pycritical_section lock((internals).mutex)
#else
#define PYBIND11_LOCK_INTERNALS(internals)
#endif
