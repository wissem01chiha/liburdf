...
struct holder_caster_foreign_helpers {
  ...
  // Downcast shared_ptr from the enable_shared_from_this base to the target
  // type. SFINAE probe: use static_pointer_cast when the static downcast is
  // valid (common case), fall back to dynamic_pointer_cast when it isn't
  // (virtual inheritance — issue #5989). We can't use dynamic_pointer_cast
  // unconditionally because it requires polymorphic types; we can't use
  // is_polymorphic to choose because that's orthogonal to virtual inheritance.
  // (The implementation uses the "tag dispatch via overload priority" trick.)
  template <typename type, typename esft_base>
  static auto esft_downcast(const std::shared_ptr<esft_base> &existing,
                            int /*preferred*/)
      -> decltype(static_cast<type *>(std::declval<esft_base *>()),
                  std::shared_ptr<type>()) {
    return std::static_pointer_cast<type>(existing);
  }

  template <typename type, typename esft_base>
  static std::shared_ptr<type> esft_downcast(
      const std::shared_ptr<esft_base> &existing, ... /*fallback*/) {
    return std::dynamic_pointer_cast<type>(existing);
  }

  template <typename type>
  static auto set_via_shared_from_this(type *value,
  ...
  template <typename type>
  static bool set_foreign_holder(handle src, const type *value,
  ...
  static bool set_foreign_holder(handle, type *, ...) {
    throw cast_error(
        "Unable to cast foreign type to held instance -- "
