...
JointParser::JointParser() { p_ = std::make_shared<Joint>(); }

int JointParser::parse(const tinyxml2::XMLElement* xml) {
  if (xml == nullptr) {
    LOG_F(ERROR, "JointParser::parse() received null pointer");
    return -1;
  }
  ...
  while (origin_xml) {
    ...
    if (!ref_str) {
      ...
      if (!p_->isParent(ref_str)) {
        LOG_F(ERROR,
              "specified parent '%s' in origin of joint '%s' does not match "
              "any known parent links.",
  ...
  const tinyxml2::XMLElement* calibration_xml =
  ...
  const tinyxml2::XMLElement* safety_xml =
