...
CameraParser::CameraParser() { p_ = std::make_shared<Camera>(); }

int CameraParser::parse(const tinyxml2::XMLElement* xml) {
  if (xml == nullptr) {
    LOG_F(ERROR, "CameraParser::parse() received null pointer");
    return -1;
  }
  ...
  if (camera_xml) {
    const tinyxml2::XMLElement* image_xml =
        camera_xml->FirstChildElement("image");
    if (image_xml) {
...
bool CameraParser::empty() const { return p_ == nullptr; }

void CameraParser::clear() { p_.reset(); }

std::string CameraParser::toString() const { return p_->toString(); }