...
DataTypeParser::DataTypeParser() { p_ = std::make_shared<DataType>(); }
...
bool DataTypeParser::isA(const char* name) const {
  return std::strcmp(name, "data_type") == 0;
}
...
std::shared_ptr<DataType> DataTypeParser::get() { return p_; }