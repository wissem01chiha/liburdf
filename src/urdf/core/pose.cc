...
std::string Pose::toString() const {
  ...
  os << "pose xyz-xyzw = [" << position[0] << ", " << position[1] << ", "
     << position[2] << ", " << rotation.coeffs()[0] << ", "
     << rotation.coeffs()[1] << ", " << rotation.coeffs()[2] << ", "
     << rotation.coeffs()[3] << "]";
  return os.str();
}
...
Vec3 Pose::getPosition() { return position; }

Rot3 Pose::getRotation() { return rotation; }
