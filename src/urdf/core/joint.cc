...
Joint::Joint(const Joint &rhs) {
  ...
  if (rhs.dynamics) {
    dynamics = std::make_shared<JointDynamics>(*rhs.dynamics);
  }
  if (rhs.limits) {
    limits = std::make_shared<JointLimits>(*rhs.limits);
  }
  if (rhs.safety) {
    safety = std::make_shared<JointSafety>(*rhs.safety);
  }
  if (rhs.calibration) {
    calibration = std::make_shared<JointCalibration>(*rhs.calibration);
  }
  if (rhs.mimic) {
    mimic = std::make_shared<JointMimic>(*rhs.mimic);
  }
}
...
std::string Joint::toString() const {
  ...
  os << "Joint : " << name.c_str() << " [Type: " << static_cast<int>(type)
...
void Joint::pushBackChild(const std::string lk) {
  if (!lk.empty()) {
    this->child.push_back(lk);
  }
}

void Joint::pushBackParent(const std::string lk) {
  if (!lk.empty()) {
    this->parent.push_back(lk);
  }
}
...
std::vector<std::string> Joint::getChild() const { return this->child; }

std::vector<std::string> Joint::getParent() const { return this->parent; }

Vec3 Joint::getAxis() const { return axis; };