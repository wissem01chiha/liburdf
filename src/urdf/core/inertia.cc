...
bool Inertia::validate() const {
  ...
  if (std::abs(ixy) > std::sqrt(ixx * iyy) ||
    ...
    LOG_F(WARNING, "Products of inertia are invalid: ixy=%f, ixz=%f, iyz=%f",
          ixy, ixz, iyz);
    return false;
  }
  return true;
...
std::string Inertia::toString() const {
  std::ostringstream os;
  os << "Inertia = ["
     << "mass=" << mass << ", ixx=" << ixx << ", ixy=" << ixy << ", ixz=" << ixz
...
void Inertia::setIxx(double ixx_) { ixx = ixx_; }

void Inertia::setIxy(double ixy_) { ixy = ixy_; }

void Inertia::setIxz(double ixz_) { ixz = ixz_; }

void Inertia::setIyy(double iyy_) { iyy = iyy_; }
