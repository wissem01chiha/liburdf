...
void init_pyurdf_parser(py::module &m) {
    py::class_<URDFParser, std::shared_ptr<URDFParser>>(m, "URDFParser")
        ...
        .def("parse", static_cast<int (URDFParser::*)(const std::string&)>(&URDFParser::parse),
             "Parse URDF from filename")
        .def("parse", static_cast<int (URDFParser::*)(const char*)>(&URDFParser::parse),
             "Parse URDF from C-string filename")
        .def("parse", static_cast<int (URDFParser::*)(const tinyxml2::XMLElement*)>(&URDFParser::parse),
